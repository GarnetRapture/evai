...
`dist/` is only static files, so it can go on any static host. From 0.0.6 there is no official hosted service; the local server is the supported way to run it. The File System Access API works only on HTTPS or localhost.
