...
- The EverTalk and story lines in `dialogues` are both voice samples and the measurement source of each spirit's temperament. `src/domains/persona/temperament.ts` counts warmth, expressiveness and initiative signals in the spirit's own lines and turns them into a percentile among all 99 spirits. Game system notices inside the lines (bracketed or parenthesized notices, "(spirit name) 님이 …") are not the spirit speaking and are left out.
- `yuria.json` holds the playable spirit Yuria Queen (SNO 5030). It was filled from `third_party/tbl_json` by following that SNO, the same way as the other spirits, and carries 285 EverTalk lines.
