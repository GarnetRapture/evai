...
- **저장소**: 일반 웹에서는 IndexedDB `eversoul-ai-chat` 하나에 모든 데이터를 두고 시작할 때 영구 저장을 요청합니다. 로컬 서버에서는 `evai-database/evai.sqlite3` 한 파일에 외래키를 갖춘 테이블(persona, chat_room, chat_message, persona_memory, persona_memory_source_message 등)로 저장하고, 정령별·정령 간 대화 내역은 `persona_conversation` 뷰로 함께 조회합니다.
