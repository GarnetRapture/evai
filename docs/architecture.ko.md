...
- **시스템 프롬프트**: 정령 원본의 프로필·성격·인사, 전체 말투/스토리/에버톡 말뭉치에서 분산 표집한 정령 발화와 실제 `구원자 → 정령` 반응쌍을 시작 정체성으로 조립합니다. 매 턴에는 DB 전체 타임라인에서 계산한 최근 3~8개 교환, 이전 구간·과거 방 개요, 관련 에피소드·지시·키워드, 감정·인연·질투, 현재 질의와 관련된 다른 정령 대화 구간, 세계관 및 정령별 데이터셋 기록을 동적으로 추가합니다. `zh_cn`은 OpenCC로 간체화하고 출력 이모지를 제거합니다.
...
- **기억**: 응답 메시지와 매 턴의 `구원자/정령` 에피소드 기억은 하나의 저장소 트랜잭션으로 저장됩니다. 매 턴 애플리케이션 로직이 전체 DB 타임라인을 다시 읽고 최근 흐름, 과거 세션 개요, 관계 변화와 다른 정령 대화를 계산합니다. 관련 에피소드는 1~3글자 n-gram을 FNV-1a로 512차원에 희소 저장한 어휘 벡터와 코사인 유사도로 찾습니다. 모델은 digest·reflection·consolidation을 생성하거나 저장하지 않고 현재 응답만 생성합니다.
