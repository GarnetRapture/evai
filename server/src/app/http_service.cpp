...
std::string runtime_body(const HttpServiceContext& context)
{
    return std::format(
        ...
        http::json_escaped(service_version),
        http::json_escaped(storage::sqlite_library_version()),
        http::json_escaped(context.database->file().string()),
        http::json_escaped(api::ollama_proxy_prefix),
        context.port);
}

void handle_storage_request(const net::TcpSocket& client, const HttpServiceContext& context, std::string_view operation, const http::HttpRequest& request)
{
    ...
    if (!request.header("Content-Type").starts_with(json_content_type_prefix)) {
        http::send_json(client, 415, "Unsupported Media Type", http::json_error_body("unsupported_media_type", request.header("Content-Type")));
        return;
    }
...
void handle_backup_request(const net::TcpSocket& client, const HttpServiceContext& context, std::string_view operation, const http::HttpRequest& request)
{
    ...
    const storage::BackupResponse response = [&]() {
        if (operation == "read") {
            return context.backups->read_file(request.body);
        }
        if (operation == "write") {
            return context.backups->write_file(request.body);
        }
        if (operation == "delete") {
...
void handle_request(const net::TcpSocket& client, const HttpServiceContext& context)
{
    ...
    if (path->starts_with(api_prefix)) {
        if (!is_allowed_origin(context, request.header("Origin"))) {
            http::send_json(client, 403, "Forbidden", http::json_error_body("forbidden_origin", request.header("Origin")));
            return;
        }
...
HttpServiceContext create_http_service_context(
