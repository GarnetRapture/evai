...
evai::server::app::ServerConfig resolve_config(const std::filesystem::path &file)
{
    ...
    {
        config.voice = evai::server::app::choose_voice_language(config.language);
...
void ensure_assets(const std::filesystem::path &root, const evai::server::app::ServerConfig &config)
{
    ...
    evai::server::app::print_asset_check(config.language);
    const evai::server::api::AssetPlan plan = evai::server::api::plan_assets(root, source, config.voice);
    if (!plan.error.empty())
    {
        evai::server::app::print_asset_summary(
            config.language, {plan.present, plan.relocated, 0, plan.pending.size(), 0, plan.error});
        evai::server::app::record_error("assets", plan.error);
        return;
    }
    std::size_t reported = 0;
    const evai::server::api::AssetOutcome outcome = evai::server::api::fetch_assets(
        root, source, plan,
        [&config, &reported](const evai::server::api::AssetProgress &progress) {
            if (progress.completed != progress.total && progress.completed - reported < 5)
            {
                return;
            }
            reported = progress.completed;
            evai::server::app::print_asset_progress(config.language, progress.completed, progress.total,
                                                    progress.bytes, progress.total_bytes);
        });
    evai::server::app::print_asset_summary(
        config.language,
        {plan.present, plan.relocated, outcome.downloaded, outcome.failed, outcome.bytes, outcome.error});
    if (outcome.failed == 0 && outcome.error.empty())
    {
        evai::server::api::store_asset_manifest(root, source, config.voice);
        return;
    }
    evai::server::app::record_error("assets", outcome.error);
}
...
int run_server(const evai::server::app::ServerOptions &options)
{
    ...
    evai::server::app::print_browser_launch(config.language, site_url,
                                            evai::server::platform::open_default_browser(site_url));
    for (;;)
    {
...
int run(const std::vector<std::string_view> &arguments)
{
    ...
    return options.run_mode == evai::server::app::ServerRunMode::create_database ? create_database(options)
                                                                                 : run_server(options);
}
