...
import type { BackupDirectoryStatus, LocalStatusSnapshot, SyncMetadataKey, SyncResult } from './types';
...
export const syncClient = {
    ...
    async importBackupFromFileForReload(): Promise<boolean> {
        const snapshot = await backupService.pickSnapshotFile();
        if (!snapshot) {
            return false;
        }
        await restoreSnapshotForReload(snapshot);
        return true;
    },
    ...
    async restoreBackupDirectoryFileForReload(fileName: string): Promise<void> {
