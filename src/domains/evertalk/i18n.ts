...
export interface EverTalkLabels {
  ...
  familiaritySigilGradeNames: Record<
    "epic" | "eternal" | "legendary" | "origin",
    string
  >;
  ...
  platformGuideItems: Record<
    AppPlatform,
    (modelSettingsPath: string) => string[]
  >;
  ...
  chromePromptVariantFlag: (
    flagUrl: string,
    requiresEnabled: boolean,
  ) => string;
  chromePromptVariantVerification: Record<
    ChromePromptVariantVerification,
    string
  >;
  chromePromptVariantInstalled: (
    modelName: string,
  ...
  chromeInstalledModelTitle: (
    modelName: string,
    modelVersion: string,
  ) => string;
  chromeInstalledModelUnlinkedTitle: string;
  chromeInstalledModelMeta: (
    store: string,
    componentVersion: string,
    megabytes: number,
    format: string,
  ...
  storyLoveTitle: string;
  storyChapterLabel: string;
  storyEpisodeLabel: string;
  storyEndingBad: string;
  storyEndingNormal: string;
  storyEndingTrue: string;
  storyRequiredAffinity: string;
  storyPrevious: string;
  storyNext: string;
  storyBackToList: string;
  storyChoicePrompt: string;
  storyLoading: string;
  ...
  bgmPrevious: string;
  bgmNext: string;
  bgmOrderListed: string;
  bgmOrderTitle: string;
  bgmOrderShuffle: string;
  bgmSearch: string;
  bgmEmpty: string;
  bgmVolume: string;
  ...
  memoryNetworkViewAll: string;
  memoryNetworkViewAllDescription: string;
  memoryNetworkOnlySpirit: string;
  memoryNetworkEmpty: string;
  memoryNetworkNodeValue: (level: number, affection: number) => string;
  ...
  memoryGraphEdgeSaviorBondLabel: (
    level: number,
    messageCount: number,
  ) => string;
  memoryGraphEdgeCanonBondLabel: (
    strength: number,
    sharedUnion: string | null,
  ) => string;
  memoryGraphEdgeRelationSaviorLabel: (level: number) => string;
  memoryGraphEdgeRivalLabel: (messageCount: number) => string;
  memoryGraphEdgeJealousyLabel: (
    firstName: string,
    firstToSecondStir: number | null,
    secondName: string,
    secondToFirstStir: number | null,
  ) => string;
  memoryGraphPersonaHeartValue: (
    level: number,
  ...
  memoryHeartContactSummary: (
    contactDays: number,
    saviorMessages: number,
  ) => string;
...
export const EVERTALK_LABELS: Record<AppLanguage, EverTalkLabels> = {
  ko: {
    ...
    languageGateDescription:
      "에버톡 UI와 로컬 모델 응답에 사용할 언어를 선택하세요.",
    languageKo: "한국어",
    languageEn: "English",
    languageZhCn: "简体中文",
    continue: "시작",
    ...
    familiarityDescription:
      "정령과 대화하면 브라우저 IndexedDB에 쌓인 메시지와 기억 누적량으로 친밀도가 산출됩니다.",
    ...
    familiarityStepHint: (step) =>
      `친밀도 ${step}점마다 별 1개가 늘어나며 상한이 없습니다.`,
    ...
    inventory: "인벤토리",
    inventoryEmpty:
      "아직 40레벨에 도달한 정령이 없습니다. 정령과 대화해 인연을 40레벨까지 키우면 인연 스티커를 얻습니다.",
    ...
    proactiveNotificationHint:
      "정령을 선택하면 먼저 건넨 대화를 확인하고 읽음 처리합니다.",
    ...
    resetDescription: {
      sqlite:
        "로컬 서버 SQLite 데이터베이스의 모든 행, 이 origin의 localStorage, 예전 버전이 남긴 이 origin의 IndexedDB를 삭제해 대화, 정령/스타일/지식팩, 기억, 모듈, 설정을 초기 상태로 되돌린 뒤 페이지를 다시 불러옵니다.",
      indexeddb:
        "현재 origin의 IndexedDB 데이터베이스 전체와 localStorage를 삭제해 대화, 정령/스타일/지식팩, 기억, 모듈, 설정 및 파일 연결을 초기 상태로 되돌린 뒤 페이지를 다시 불러옵니다. 다른 탭이 데이터베이스를 붙잡고 있으면 삭제를 중단하고 오류를 표시합니다.",
    },
    resetFailed: "초기화 실패",
    notConfigured: "미지정",
    resetting: "초기화 중...",
    ...
    firstMessageHint:
      "첫 메시지부터 대화가 누적됩니다. 일반 웹에서는 이 브라우저의 IndexedDB에, EVAI 로컬 서버에서는 서버 폴더의 SQLite 데이터베이스에 저장됩니다.",
    messagePlaceholder: (name) => `${name}에게 메시지를 입력하세요...`,
    modelRequiredPlaceholder:
      "설정 > 온디바이스 모델 목록에서 모델을 준비하세요",
    send: "전송",
    ...
    bondDescription:
      "정령과 대화를 나누면 실제 메시지/기억 누적량을 기준으로 랭킹이 산출됩니다.",
    ...
    emptyProfilePanel:
      "정령을 선택하면 TBL 기반 프로필과 원본 자산 연결 상태가 표시됩니다.",
    ...
    roomMessageCount: (rooms, messages) =>
      `${rooms}개 대화방 · ${messages}개 메시지`,
    ...
    platformGuideCheckbox: {
      web_chrome:
        "위 안내와 Chrome 온디바이스 모델·브라우저 저장 방식을 확인했습니다.",
      android_app:
        "위 안내를 확인했으며, 이 앱은 기기에 설치한 LiteRT-LM 모델로만 동작함을 이해했습니다.",
    },
    platformGuideConfirm: "확인하고 입장",
    platformBlockedTitle: "지원하지 않는 이용 환경입니다",
    platformBlockedMessages: {
      mobile_device:
        "모바일 웹은 지원하지 않습니다. PC 데스크톱 브라우저 또는 전용 Android 앱을 이용해 주세요.",
      unsupported_browser:
        "이 데스크톱 환경을 확인할 수 없습니다. Chrome·Edge·Firefox·Whale 등 최신 데스크톱 브라우저를 이용해 주세요.",
    },
    platformBlockedHint:
      "지원 환경: Windows · macOS · Linux · ChromeOS의 최신 데스크톱 브라우저",
    messageSendFailed: "응답 생성에 실패했습니다. 다시 시도해 주세요.",
    modelListTitle: "대화 모델",
    modelListDescription: {
      web_chrome:
        "Chrome Prompt API 모델(Gemini Nano · Gemma 4)과 Chrome이 설치한 온디바이스 모델을 선택할 수 있습니다. EVAI 로컬 서버로 열었을 때는 Ollama에 설치된 모델(Ollama 라이브러리·Hugging Face GGUF·GGUF 파일로 만든 모델)이 로컬 Ollama 모드에 함께 표시되며, 선택한 모델이 대화에 고정됩니다.",
      android_app:
        "이 기기에서 Google LiteRT-LM 엔진으로 실행하는 온디바이스 AI 모델입니다. 대화에 사용할 모델을 설치하고 선택하세요. 모델을 불러올 때 GPU 백엔드를 먼저 시도하고, 사용할 수 없으면 CPU 백엔드로 실행합니다.",
    },
    modelRoleChat: "대화 생성 · Chrome Prompt API (브라우저 내장 모델)",
    modelInputModalities: (image, audio) =>
      `입력 지원 · 텍스트 · 이미지 ${image} · 오디오 ${audio}`,
    modelSamplingParams: (
      defaultTopK,
      maxTopK,
      defaultTemperature,
      ...
      `샘플링 · topK 기본 ${defaultTopK} / 최대 ${maxTopK} · 온도 기본 ${defaultTemperature} / 최대 ${maxTemperature}`,
    modelProbeFailed: (detail) => `브라우저 모델 정보 조회 실패: ${detail}`,
    modelSamplingParamsWebUnavailable:
      "샘플링 파라미터 · 이 브라우저는 웹 페이지에 LanguageModel.params()를 제공하지 않음 (samplingMode로 제어)",
    ...
    chromeBuiltInAiApiState: (apiName, languagePair, state) =>
      `Chrome 내장 AI · ${apiName}${languagePair ? ` ${languagePair}` : ""} · ${state}`,
    chromeBuiltInAiApiNotExposed: "이 브라우저에 노출되지 않음",
    chromeBuiltInAiApiFailed: (detail) => `상태 확인 실패 (${detail})`,
    chromeBuiltInAiInventoryReadAt: (readAt) =>
      `Chrome 내장 AI 상태 확인 시각 · ${readAt}`,
    chromeOnDeviceInventoryUnavailable: (detail) =>
      `Chrome 내장 AI 상태를 읽지 못함 (${detail})`,
    chromePromptVariantTitle: {
      nano: "대화 생성 · Chrome Prompt API · Gemini Nano (기본)",
      gemma4: "대화 생성 · Chrome Prompt API · Gemma 4 (플래그 전환)",
    },
    chromePromptVariantFlag: (flagUrl, requiresEnabled) =>
      `필요한 Chrome 플래그 · ${flagUrl} = ${requiresEnabled ? "Enabled" : "Default"} (변경 후 Chrome 재시작)`,
    chromePromptVariantVerification: {
      active: "Chrome 플래그 상태 확인됨 · 이 모델로 실행됩니다",
      flag_mismatch:
        "Chrome 플래그 상태가 이 모델과 다름 · 플래그를 바꾸고 재시작한 뒤 Local State를 다시 연결하세요",
      unverified:
        "Chrome 플래그 상태 미확인 · 아래에서 Local State 파일을 연결하면 검증합니다",
    },
    chromePromptVariantInstalled: (modelName, componentVersion, megabytes) =>
      `설치됨 · ${modelName} · 컴포넌트 ${componentVersion} · ${megabytes} MB`,
    chromePromptVariantAsset: (assetId, version) =>
      `Chrome 모델 기록 · ${assetId} · ${version}`,
    chromePromptVariantLastUsed: (usedAt) =>
      `Chrome에서 마지막 사용 · ${usedAt}`,
    chromeBrowserModelState: (gemma4FlagEnabled, chromeVersion, readAt) =>
      `Chrome 플래그 상태 · Gemma 4 ${gemma4FlagEnabled ? "사용(Enabled)" : "기본(Nano)"}${chromeVersion ? ` · Chrome ${chromeVersion}` : ""} · 확인 ${readAt}`,
    chromeBrowserModelStateMissing:
      'Chrome 플래그 상태를 아직 읽지 않았습니다. "Local State 파일 선택"으로 연결하세요.',
    ...
    chromeInstalledModelSectionDescription:
      "Chrome 온디바이스 AI는 기본으로 Gemini Nano를 쓰고, gemma4-for-built-in-ai 플래그를 켜면 Gemma 4로 전환됩니다. 모델 폴더와 Local State를 연결하면 설치된 모델과 현재 플래그 상태가 표시되고, 위 대화 모델 선택의 Prompt API 모델이 실제 플래그와 맞는지 검증됩니다. 연결된 LiteRT-LM 형식 Gemma는 위 대화 모델 선택의 온디바이스 AI 모드에 나타나며, 고르면 이 앱이 가중치를 직접 실행합니다.",
    chromeInstalledModelEmpty:
      "아직 연결된 모델 폴더가 없습니다. 아래에서 경로를 저장하고 모델 폴더를 선택하세요.",
    chromeInstalledModelTitle: (modelName, modelVersion) =>
    ...
    chromeInstalledModelMeta: (
      ...
      `${store} · 컴포넌트 ${componentVersion} · ${megabytes} MB · 가중치 ${format === "litertlm" ? "LiteRT-LM" : "Chrome 전용 형식"}${performanceHints.length > 0 ? ` · 성능 힌트 ${performanceHints.join(", ")}` : ""}`,
    chromeInstalledModelRelinkRequired:
      "선택은 저장됨 · 이번 세션에서 모델 폴더를 다시 선택해야 실행됩니다",
    chromeInstalledModelNotRunnable:
      "Chrome 전용 형식이라 이 앱에서 직접 실행할 수 없음 (Chrome Prompt API로만 사용 가능)",
    chromeInstalledModelRunnable:
      "연결됨 · 대화 모델 선택의 온디바이스 AI 모드에서 고를 수 있음",
    chromeInstalledModelPathLabel: "브라우저 사용자 데이터 폴더 경로",
    chromeInstalledModelPathPlaceholder:
      "C:\\Users\\사용자\\AppData\\Local\\Google\\Chrome\\User Data",
    chromeInstalledModelPathHint:
      "Chrome, Edge 등 브라우저마다 경로가 다릅니다. 저장하면 모델 폴더 경로가 아래에 표시되며, 복사해 폴더 선택 창 주소창에 붙여 넣으면 됩니다.",
    ...
    ollamaModelSectionDescription:
      "EVAI 로컬 서버가 중계할 Ollama의 연결 상태와 주소를 관리합니다. Ollama에 설치된 모든 모델은 위 대화 모델 선택의 로컬 Ollama 모드에 나타나며, 대화·기억·페르소나 규칙은 온디바이스 AI 모드와 동일하게 적용됩니다.",
    ollamaServerConnected: (version) => `Ollama 연결됨 · 버전 ${version}`,
    ollamaServerUnavailable:
      "Ollama에 연결되지 않음 · Ollama 실행 여부와 EVAI 로컬 서버의 Ollama 주소를 확인하세요",
    ollamaModelEmpty:
      "Ollama에 설치된 모델이 없습니다. 터미널에서 ollama pull 로 모델을 받으세요.",
    ollamaModelMeta: (family, parameterSize, quantization, megabytes) =>
      `${family} · ${parameterSize} · ${quantization} · ${megabytes} MB`,
    ollamaBaseUrlLabel: "Ollama 주소",
    ollamaBaseUrlPlaceholder: "http://127.0.0.1:11434",
    ollamaBaseUrlHint:
      "EVAI 로컬 서버가 중계할 Ollama 주소입니다. 경로 없이 http 프로토콜·호스트·포트만 입력합니다. 기본값은 http://127.0.0.1:11434 입니다. 저장한 값은 서버의 SQLite 설정에 기록되고, 서버 중계와 서버 콘솔의 Ollama 연결 점검이 모두 이 한 값만 사용합니다.",
    ollamaBaseUrlSave: "주소 저장",
    generationLimitsTitle: "컨텍스트와 응답 길이",
    generationLimitsDescription:
      "여기에 적은 값만큼 모델을 올리고 그만큼 씁니다. 비워두면 엔진이 보고하는 값을 그대로 씁니다.",
    generationLimitsContextLabel: "컨텍스트 토큰",
    generationLimitsContextHint:
      "Ollama는 이 값으로 num_ctx를 지정해 모델을 다시 올립니다. 모델 최대치를 넘으면 최대치로 내려 맞춥니다.",
    generationLimitsOutputLabel: "응답 토큰",
    generationLimitsOutputHint:
      "한 번의 답변에 생성할 최대 토큰 수입니다. 컨텍스트에서 이만큼을 응답 몫으로 남깁니다.",
    generationLimitsAutoPlaceholder: "자동",
    generationLimitsInvalid: "1 이상의 정수를 적거나 비워 주세요.",
    generationLimitsModelMaximum: (tokens) =>
      `모델 최대 ${tokens.toLocaleString("ko-KR")} 토큰`,
    generationLimitsActive: (tokens) =>
      `현재 적용 ${tokens.toLocaleString("ko-KR")} 토큰`,
    ...
    ollamaGuideDescription:
      'EVAI 로컬 서버가 이 PC의 Ollama로 요청을 중계합니다. 아래 명령으로 Ollama를 준비하고 로컬 서버를 실행한 뒤 "연결 확인"을 누르세요. 연결된 모델은 셋업과 설정의 대화 모델에서 로컬 Ollama 모드로 선택하며, 선택한 모델이 대화에 고정됩니다.',
    ollamaConnectionChecking: "Ollama 연결 확인 중…",
    ollamaConnectionNotChecked: "아직 Ollama 연결을 확인하지 않았습니다",
    ollamaConnectionReady: (version, modelCount) =>
      modelCount > 0
    ...
    localServerNoticeDescription:
      "지금은 일반 웹으로 열려 있어 Chrome 온디바이스 AI만 사용할 수 있고, 데이터는 이 브라우저 IndexedDB에 저장됩니다. 저장소에서 EVAI 로컬 서버를 받아 실행하면 로컬 Ollama 모드(Ollama 라이브러리·Hugging Face GGUF 모델)와 SQLite 데이터베이스를 함께 쓸 수 있습니다.",
    ...
    localServerConnected: (version, sqliteVersion) =>
      `EVAI 로컬 서버 연결됨 · 버전 ${version} · SQLite ${sqliteVersion}`,
    localServerDatabasePath: (databasePath) =>
      `데이터베이스 파일 · ${databasePath}`,
    storageBackendName: {
    ...
    ollamaCommandStepDescriptions: {
      verify_install:
        "Ollama를 설치한 뒤 버전 번호가 출력되는지 확인합니다. 명령을 찾지 못하면 터미널을 닫았다가 다시 열고, 그래도 안 되면 Ollama를 다시 설치하세요.",
      pull_model:
        "입력한 모델을 Ollama 라이브러리나 Hugging Face(hf.co/…)에서 받습니다. PC 사양에 맞는 어떤 모델이든 사용할 수 있습니다.",
      create_from_gguf:
        "입력한 GGUF 파일(또는 Ollama blobs의 sha256 파일)을 FROM으로 지정해 입력한 이름의 모델을 만듭니다.",
      run_model:
        "모델을 한 번 실행해 동작을 확인하고(/bye 로 종료), ollama ls 로 설치 목록을, ollama ps 로 실행 중인 모델을 확인합니다. 앱에서는 대화 모델 선택의 로컬 Ollama 모드에서 직접 고른 모델만 사용합니다.",
      remove_model: "더 이상 쓰지 않는 모델을 지웁니다.",
      expose_network:
        'Ollama와 EVAI 로컬 서버가 같은 PC에 있으면 이 단계는 필요 없습니다. 다른 PC에서 실행 중인 Ollama를 쓸 때만 그 PC에서 Ollama 설정의 "Expose Ollama to the network"를 켜거나 OLLAMA_HOST를 0.0.0.0:11434로 지정해 다시 실행하고, 설정 > 대화 모델의 Ollama 주소를 그 PC 주소로 바꿉니다.',
      run_local_server:
        "웹 번들과 같은 폴더에서 EVAI 로컬 서버를 실행하고 http://127.0.0.1:9999/ 를 엽니다. 브라우저는 이 서버를 통해서만 Ollama와 통신합니다.",
    },
    ollamaCommandCopy: "명령 복사",
    ollamaRecommendedModelTitle: (modelName) => `추천 모델: ${modelName}`,
    ollamaRecommendedModelHint:
      "터미널에서 아래 명령으로 모델을 받은 뒤 새로고침을 누르면 목록에 나타납니다.",
    ollamaGuideModelNameLabel: "사용할 모델 이름",
    ollamaGuideModelNamePlaceholder:
      "모델이름:태그 또는 hf.co/사용자/저장소:양자화",
    ollamaGuideModelNameHint:
      "입력한 이름으로 받기·실행·삭제 명령이 만들어집니다. 이미 설치된 모델은 아래 버튼으로 고를 수 있습니다.",
    ollamaGuideGgufPathLabel: "GGUF 파일 경로 (선택)",
    ollamaGuideGgufPathPlaceholder:
      "PC에 있는 .gguf 또는 blobs\\sha256-… 파일의 절대 경로",
    ollamaGuideGgufPathHint:
      "경로를 입력하면 그 파일로 위 이름의 모델을 만드는 명령이 추가됩니다.",
    modelRoleAndroidGeminiNano: "대화 생성 · Android AICore (Gemini Nano)",
    modelAndroidGeminiNanoUnsupported:
      "이 기기는 AICore Gemini Nano를 지원하지 않습니다 (Android 12 이상 · AICore 지원 기기 필요)",
    modelLanguageSupport: (languageTag, declared) =>
      declared
    ...
    localModelSections: {
      litert_lm: {
        ...
        description:
          "Google LiteRT-LM 엔진으로 Hugging Face litert-community의 .litertlm 모델을 이 기기 안에서 실행합니다. 모델 파일은 앱 내부 저장소에만 보관되며, 대화·기억·페르소나·이모지 금지 규칙은 똑같이 적용됩니다.",
    ...
    localModelGated:
      "Hugging Face 로그인과 모델 약관 동의 후 다운로드할 수 있습니다",
    localModelBackend: (backend) => `백엔드 ${backend}`,
    localModelFileMeta: (fileName, sizeMb, license) =>
      [
        fileName,
        sizeMb === null ? null : `${sizeMb}MB`,
        license === null ? null : `라이선스 ${license}`,
      ]
    ...
    modelSessionDetail: (
      ...
      `${personaId} · 컨텍스트 ${cachedTokens}/${contextWindow} · 재사용 ${reusedTokens}`,
    modelRequestDetail: (
      state,
      promptTokens,
      generatedTokens,
      ...
      `${state} · 입력 ${promptTokens ?? "-"} · 생성 ${generatedTokens ?? "-"} · 잘림 ${truncatedTokens}`,
    backupTitle: "데이터 저장 · 불러오기",
    backupDescription: {
      sqlite:
        "백업을 누를 때만 서버가 SQLite 데이터베이스의 일관된 복사본(스냅샷)을 만들어 evai-backup 폴더에 .sqlite3 파일로 남깁니다. 자동 백업은 없으며, 복원은 선택한 스냅샷을 현재 데이터베이스와 그대로 교체합니다.",
      indexeddb:
        "백업을 누를 때만 이 브라우저 IndexedDB의 직렬화 가능한 데이터(대화, 기억, 설정, 모듈 등)를 JSON 파일로 내보내고, 불러올 때는 검증 후 한 번의 트랜잭션으로 교체한 뒤 페이지를 다시 불러옵니다. 자동 백업은 없습니다.",
    },
    backupStorageScope: {
      sqlite:
        "스냅샷 파일은 서버 SQLite 데이터베이스 전체를 담으므로 그대로 복원할 수 있습니다. localStorage와 백업 권한은 제외됩니다.",
      indexeddb:
        "JSON은 현재 브라우저 IndexedDB의 직렬화 가능한 데이터만 교체 복원합니다. localStorage는 제외됩니다.",
    },
    ...
    backupFolderTitle: "백업",
    backupFolderDescription:
      "자동 백업은 없습니다. 지금 백업을 누를 때만 백업 파일이 만들어지고, 목록에서 원하는 시점을 골라 그대로 복원할 수 있습니다. 최근 10개가 유지됩니다.",
    backupFolderLinked: (name) => `백업 위치: ${name}`,
    backupFolderNotLinked: "백업을 사용할 수 없습니다.",
    backupFolderLink: "백업 열기",
    ...
    backupLastError: (message) => `백업 실패: ${message}`,
    backupFilesEmpty: "아직 백업 파일이 없습니다.",
    backupFileMeta: (dateTime, sizeKb) => `${dateTime} · ${sizeKb}KB`,
    backupFileRestore: "복원",
    backupRestoreConfirm: (fileName) =>
      `${fileName}(으)로 복원하면 현재 데이터가 모두 교체됩니다. 계속할까요?`,
    backupWritten: (fileName) => `${fileName} 백업을 저장했습니다.`,
    resetStorageScope: {
      sqlite:
        "로컬 서버 SQLite 데이터베이스와 이 origin의 localStorage를 초기화하고, 이 origin에 남은 IndexedDB를 삭제합니다.",
      indexeddb:
        "현재 origin의 localStorage와 IndexedDB 데이터베이스 전체를 초기화합니다.",
    },
    navChat: "대화",
    navRanking: "인연 순위",
    navMemory: "기억 흐름",
    navStory: "스토리",
    navStorage: "저장소",
    storyMainTitle: "메인 스토리",
    storyLoveTitle: "인연 스토리",
    storyChapterLabel: "챕터",
    storyEpisodeLabel: "에피소드",
    storyEndingBad: "배드 엔딩",
    storyEndingNormal: "노말 엔딩",
    storyEndingTrue: "트루 엔딩",
    storyRequiredAffinity: "필요 호감도",
    storyPrevious: "이전",
    storyNext: "다음",
    storyBackToList: "목록으로",
    storyChoicePrompt: "구원자의 선택",
    storyLoading: "스토리를 불러오는 중…",
    storyLoadFailed: "스토리를 불러오지 못했습니다.",
    storyEmpty: "표시할 스토리가 없습니다.",
    storyProgress: "진행",
    storyAuto: "자동",
    storyLog: "로그",
    storyVoice: "음성",
    ...
    guidePageDescription:
      "AI 모델이 무엇인지부터 정령과 첫 대화를 나누기까지, 지금 내 PC 상태에 맞춰 순서대로 안내합니다.",
    guideBeginnerTitle: "처음이라면 여기부터 읽어 주세요",
    guideBeginnerIntro:
      '에버톡은 인터넷의 AI 서비스가 아니라 내 PC 안에서 돌아가는 AI로 정령의 대답을 만듭니다. 대화 내용이 외부로 전송되지 않는 대신, 대답을 만들어 줄 AI 모델을 PC에 준비해야 합니다. 아래 용어를 한 번 읽고, "지금 내 상태로 따라하기"의 단계를 위에서부터 차례대로 진행하면 됩니다.',
    guideConcepts: [
      {
        term: "AI 모델 (LLM)",
        description:
          "글을 읽고 이어서 답을 써 주는 인공지능 파일입니다. 정령의 성격·말투·기억은 에버톡이 정리해서 모델에게 건네고, 모델은 그 내용에 맞춰 정령의 대답을 씁니다.",
      },
      {
        term: "로컬 LLM",
        description:
          "회사 서버가 아니라 내 PC의 그래픽카드(GPU)와 메모리로 직접 돌리는 AI 모델입니다. 사용료가 없고 대화가 PC 밖으로 나가지 않지만, PC 성능에 따라 답변 속도와 쓸 수 있는 모델 크기가 달라집니다.",
      },
      {
        term: "Chrome 온디바이스 AI",
        description:
          "PC용 Chrome에 들어 있는 AI(기본 Gemini Nano, 플래그를 켜면 Gemma 4)입니다. 프로그램을 따로 설치할 필요가 없고, 처음 한 번 Chrome이 모델을 내려받습니다. 가장 쉽게 시작하는 방법입니다.",
      },
      {
        term: "Ollama (올라마)",
        description:
          "로컬 LLM을 내려받고 실행해 주는 무료 프로그램입니다. 설치하면 백그라운드에서 켜져 있고, 명령어 한 줄로 원하는 모델을 받습니다. Chrome 내장 모델보다 크고 표현력이 좋은 모델을 고를 수 있습니다.",
      },
      {
        term: "모델 이름과 크기",
        description:
          'Ollama 모델은 "이름:태그" 형태로 부릅니다. 태그의 4b, 9b 같은 숫자는 모델 규모(40억, 90억 개 매개변수)로, 클수록 똑똑하지만 메모리를 더 씁니다. q4, Q4_K_M 같은 표시는 용량을 줄인 버전입니다. 처음에는 모델 파일 크기가 그래픽카드 메모리(VRAM)보다 작은 것을 고르세요.',
      },
      {
        term: "Hugging Face · GGUF",
        description:
          "Hugging Face는 AI 모델이 공개되는 사이트이고, GGUF는 로컬 LLM용 모델 파일 형식입니다. Hugging Face의 GGUF 모델은 ollama pull hf.co/사용자/저장소 명령으로 Ollama에 바로 받을 수 있고, 이미 가진 GGUF 파일도 Ollama 모델로 만들 수 있습니다.",
      },
      {
        term: "EVAI 로컬 서버",
        description:
          "에버톡 화면을 내 PC에서 띄워 주는 작은 프로그램(evai-server)입니다. 이 서버로 열어야 Ollama 모델을 쓸 수 있고, 대화·기억·설정이 서버 폴더의 SQLite 데이터베이스에 저장됩니다. 일반 웹 주소로 열면 Chrome 온디바이스 AI만 쓸 수 있고 데이터는 이 브라우저(IndexedDB)에 저장됩니다.",
      },
    ],
    ...
    guideChecklistIntro: {
      ...
      local_server:
        '지금은 EVAI 로컬 서버로 열려 있어 Ollama 모델을 쓸 수 있습니다(Chrome 온디바이스 AI도 함께 쓸 수 있습니다). 각 단계의 완료 표시는 이 PC의 실제 상태를 읽어서 보여 주며, 단계를 마친 뒤 "상태 새로고침"을 누르면 다시 확인합니다.',
    },
    guideStepTitles: {
    ...
    guideStepDescriptions: {
      use_pc_chrome:
        '온디바이스 AI는 Windows·macOS·Linux의 최신 PC용 Chrome에서만 동작합니다. 여유 저장 공간 22GB 이상, 그래픽카드 메모리 4GB 초과(또는 RAM 16GB 이상)가 필요합니다. 완료로 바뀌지 않으면 Chrome을 최신 버전으로 업데이트한 뒤 "상태 새로고침"을 누르세요.',
      prepare_on_device:
        '"설정 열기" > 대화 모델에서 Gemini Nano 항목의 "다운로드 및 준비"를 누르면 Chrome이 모델을 내려받습니다. 크기가 커서 몇 분 걸릴 수 있으며, 상태가 "사용 가능"이 되면 완료입니다.',
      get_local_server:
        '"저장소 열기"에서 evai-server를 내려받아 실행한 뒤 브라우저로 http://127.0.0.1:9999/ 를 열면, 이 가이드가 Ollama 설치 단계로 바뀝니다.',
      run_local_server:
        "지금 이 화면이 EVAI 로컬 서버에서 열려 있으므로 완료입니다. 서버 창을 닫으면 앱이 멈추니 사용하는 동안 켜 두세요.",
      install_ollama:
        '"Ollama 내려받기"로 공식 사이트에서 설치 파일을 받아 설치합니다. 설치가 끝나면 Ollama가 자동으로 켜집니다(Windows는 작업 표시줄 오른쪽 알림 영역에 Ollama 아이콘이 보입니다). 그다음 "상태 새로고침"을 누르면 연결을 확인합니다.',
      pull_model:
        '아래 "명령어 입력하는 법"대로 터미널을 열고 ollama pull 모델이름 을 입력합니다. 모델은 "Ollama 모델 찾아보기"나 "Hugging Face GGUF 안내"에서 고르고, 아래 연결 가이드의 "사용할 모델 이름"에 넣으면 명령이 자동으로 만들어집니다. 받기가 끝나면 "상태 새로고침"을 누르세요.',
      select_model:
        "셋업 화면이나 설정 > 대화 모델에서 온디바이스 AI 모드 또는 로컬 Ollama 모드를 고른 뒤 모델 하나를 선택합니다. 고른 모델은 저장되어 모든 화면에 똑같이 적용됩니다.",
      start_chat:
        '모델이 "작동 중"이 되면 준비 끝입니다. 로비나 대화 화면에서 정령을 골라 말을 걸어 보세요. 첫 대답은 모델을 불러오느라 조금 늦을 수 있습니다.',
    },
    guideStepStates: {
      checking: "확인 중",
      done: "완료",
    ...
    chatModelSelectorDescription:
      "온디바이스 AI 모드와 로컬 Ollama 모드 중 하나를 고르고, 그 모드에서 실제로 연결된 모델을 선택합니다. 셋업과 설정은 같은 목록과 같은 저장값을 사용합니다.",
    chatModelModeTitles: {
      on_device: "온디바이스 AI 모드",
      ollama: "로컬 Ollama 모드",
    },
    chatModelRuntimeStates: {
      checking: "확인 중",
      running: "작동 중",
      ready: "사용 가능",
    ...
    chatModelModeEmpty: {
      on_device:
        "이 브라우저에서 지금 선택할 수 있는 온디바이스 모델이 없습니다. 설정 > 대화 모델에서 Chrome 온디바이스 모델을 준비하거나 가이드를 확인하세요.",
      ollama:
        "Ollama에서 선택할 수 있는 모델이 없습니다. 가이드의 명령으로 Ollama를 실행하고 모델을 받은 뒤 새로고침하세요.",
    },
    chatModelActiveMode: "현재 대화 모드",
    chatModelOptionCount: (count) => `선택 가능 ${count}개`,
    chatModelOllamaLocalServerOnly:
      "로컬 Ollama 모드는 EVAI 로컬 서버로 열었을 때만 사용할 수 있습니다. 실행 방법은 가이드에 있습니다.",
    chatModelSavedTo: (storageName) =>
      `선택한 모델은 ${storageName}에 저장되고 모든 화면에 같은 값으로 적용됩니다.`,
    spiritActionNote: (spiritName, actions) =>
      `${spiritName}의 행동: ${actions}`,
    proactiveMessages: "정령이 먼저 말 걸기",
    proactiveMessagesDescription:
      "켜면 대화 중인 정령과 다른 정령 모두, 대화로 쌓인 그리움·애정·질투와 성격에 따라 먼저 메시지를 보냅니다. 적극적인 정령은 답장이 올 때까지 여러 번 보낼 수 있습니다.",
    cheatMode: "치트모드",
    cheatModeDescription:
      "켜면 상단에 치트모드 화면이 생기고, 정령별 인연 레벨·성격·감정·말투 프리셋이 실제 응답에 적용됩니다.",
    cheatPageTitle: "정령 치트 설정",
    cheatPageDescription:
      "정령마다 인연 레벨을 직접 정하고, 기본 성격·감정·말하는 방식을 프리셋으로 바꿉니다. 설정은 다음 응답부터 바로 반영됩니다.",
    cheatSearchPlaceholder: "정령 이름 또는 영문명",
    cheatNoSpirits: "조건에 맞는 정령이 없습니다.",
    cheatAppliedBadge: "적용 중",
    cheatBondTitle: "인연 레벨",
    cheatBondDescription:
      "대화 기록과 무관하게 관계 단계와 인연 표시가 이 레벨로 고정됩니다.",
    ...
    setupModelSelectionHint:
      "대화에 쓸 모드와 모델을 지금 고릅니다. 여기서 고른 값은 설정 > 대화 모델과 같은 저장값입니다.",
    setupModelRequired: "모델을 하나 선택해야 시작할 수 있습니다.",
    setupModelLoading:
      "선택한 모델이 완전히 로드되어 작동 중이 되면 시작할 수 있습니다.",
    setupModelLoadFailed: (detail) =>
      `선택한 모델을 로드하지 못해 시작할 수 없습니다. 모델을 다시 선택하거나 목록을 새로 고치세요. (${detail})`,
    storageObjectKinds: {
      object_store: "오브젝트 스토어",
      table: "테이블",
      view: "뷰",
    },
    ...
    rankingPageDescription:
      "누적 대화와 기억에서 계산된 실제 인연 점수를 모든 정령과 함께 비교합니다.",
    bondScoreLabel: "인연 점수",
    memoryPageTitle: "기억 알고리즘 흐름",
    memoryPageDescription:
      "DB에 저장된 전체 대화에서 키워드 우선순위·다른 정령과의 관계·내면 상태를 분석해 다음 응답의 행동 절차를 만드는 실제 흐름입니다.",
    memoryGraphConnections: "연결선",
    memoryContextKinds: {
      conversation: "DB 대화 흐름",
      directive: "사용자 지시",
      episodic: "대화 사건",
      habit: "키워드 스레드",
      affect: "감정·질투",
    ...
    memoryFilterDescription:
      "끈 종류는 정령의 다음 응답 컨텍스트에서 제외됩니다.",
    memoryFilterSearchPlaceholder: "키워드 검색",
    memoryFilterEmpty:
      "표시할 키워드가 없습니다. 정령과 대화를 나누면 키워드 그래프가 쌓입니다.",
    memoryGraphRecentOnly: "최근 대화 맥락 키워드만",
    memoryGraphNoSpirit: "정령을 선택하면 그 정령의 기억 그래프를 분석합니다.",
    memoryGraphSelectHint:
      "휠로 확대·축소, 빈 곳을 끌어 이동, 노드를 끌어 배치합니다. 키워드나 정령을 누르면 그때 나눈 대화와 정령의 행동이 표시됩니다.",
    memoryNetworkViewAll: "전체 관계도",
    memoryNetworkViewAllDescription:
      "대화 기록이 있는 모든 정령을 한 화면에서 보고, 정령끼리 주고받는 질투까지 한 번에 확인합니다.",
    memoryNetworkOnlySpirit: "정령별 보기",
    memoryNetworkEmpty:
      "아직 관계도를 그릴 기록이 없습니다. 정령과 대화하면 전체 관계도가 채워집니다.",
    memoryNetworkNodeValue: (level, affection) =>
      `Lv.${level} · 애정 ${affection}`,
    memorySpiritRosterTitle: "분석할 정령",
    memorySpiritRosterSearch: "정령 이름 검색",
    memorySpiritRosterEmpty: "검색과 일치하는 정령이 없습니다.",
    memorySpiritRosterMeta: (level, messageCount) =>
      `Lv.${level} · 메시지 ${messageCount}`,
    memoryGraphLegend: {
      query: "방금 입력·언급과 연결",
      recent: "최근 대화 맥락",
      history: "누적 기억",
    ...
    memoryGraphKeywordCounts: (userCount, spiritCount) =>
      `구원자 ${userCount} · 정령 ${spiritCount}`,
    memoryGraphSaviorValue: (messageCount) => `나눈 메시지 ${messageCount}`,
    memoryGraphRelationValue: (saviorLevel) =>
      saviorLevel === null
        ? "구원자와 인연 없음"
        : `구원자 인연 Lv.${saviorLevel}`,
    ...
    memoryGraphEdgeSaviorBondLabel: (level, messageCount) =>
      `인연 Lv.${level} · 메시지 ${messageCount}`,
    memoryGraphEdgeCanonBondLabel: (strength, sharedUnion) =>
      sharedUnion === null
        ? `원작 인연 ${strength}`
        : `같은 소속 ${sharedUnion} · 인연 ${strength}`,
    memoryGraphEdgeRelationSaviorLabel: (level) => `구원자 인연 Lv.${level}`,
    memoryGraphEdgeRivalLabel: (messageCount) =>
      `질투 · 메시지 ${messageCount}`,
    memoryGraphEdgeJealousyLabel: (
      firstName,
      firstToSecondStir,
      ...
      [
        firstToSecondStir === null
          ? null
          : `${firstName}→${secondName} ${firstToSecondStir}%`,
        secondToFirstStir === null
          ? null
          : `${secondName}→${firstName} ${secondToFirstStir}%`,
      ]
        .filter((part) => part !== null)
        .join(" · "),
    memoryGraphPersonaHeartValue: (level, affection) =>
      affection === null ? `Lv.${level}` : `Lv.${level} · 애정 ${affection}`,
    memoryGraphJealousyNodeValue: (strongestStir) =>
      `질투 최대 ${strongestStir}%`,
    memoryHeartTitle: "감정 기복",
    memoryHeartDescription: (spiritName) =>
      `${spiritName}와(과) 나눈 대화 기록을 날짜별로 누적해 계산한 내면 상태입니다. 먼저 말 걸기와 질투 판단에 그대로 쓰입니다.`,
    memoryHeartMetrics: {
      affection: "애정",
      trust: "신뢰",
      longing: "그리움",
    ...
    memoryHeartExpressions: {
      openness: "마음 열림",
      outward_warmth: "겉으로 드러나는 온기",
      receptiveness: "받아주는 정도",
      initiative: "먼저 다가가는 정도",
    },
    memoryHeartChartLabel: (spiritName) =>
      `${spiritName}의 날짜별 애정·신뢰·그리움·상처 변화`,
    memoryHeartRivalBarLabel: "구원자가 다른 정령과 나눈 메시지",
    memoryHeartRivalCount: (count) => `다른 정령 메시지 ${count}`,
    memoryHeartContactSummary: (contactDays, saviorMessages) =>
      `대화한 날 ${contactDays}일 · 구원자 메시지 ${saviorMessages}`,
    ...
    memoryRelationCanonStats: (interactionCount, mentionCount) =>
      `원작 속 교류 ${interactionCount}회 · 언급 ${mentionCount}회`,
    memoryRelationSaviorBond: (level, messageCount) =>
      `구원자와의 인연 Lv.${level} · 나눈 메시지 ${messageCount}개`,
    memoryRelationNoSaviorBond: "아직 구원자와 대화한 기록이 없습니다.",
    memoryKeywordDetailTitle: (token) => `키워드 · ${token}`,
    memoryKeywordStats: (userCount, spiritCount, firstSeen, lastSeen) =>
      `구원자 ${userCount}회 · 정령 ${spiritCount}회 · 처음 ${firstSeen} · 최근 ${lastSeen}`,
    ...
    memoryRivalStats: (userCount, spiritCount, firstAt, latestAt) =>
      `구원자가 보낸 메시지 ${userCount}개 · 답장 ${spiritCount}개 · ${firstAt} ~ ${latestAt}`,
    ...
    memoryRivalCanonBond: (addressForm) =>
      `원작에서 부르는 호칭: ${addressForm}`,
    memoryRivalSharedUnion: (union) => `같은 소속: ${union}`,
    memoryBehaviorStages: {
      input: { title: "구원자의 입력", description: "방금 받은 말·행동·묘사" },
      keywords: {
        title: "키워드 스레드",
        description: "입력·최근 맥락과 연결된 우선 키워드",
      },
      recall: {
        title: "기억 회상",
        description: "통합 기억과 관련 대화 사건(시간순)",
      },
      social: {
        title: "다른 정령 분석",
        description: "질투·경쟁을 만드는 대화 기록",
      },
      inner_state: {
        title: "내면 상태",
        description: "정령이 직접 정리한 마음과 다음 의도",
      },
    ...
    modulesSectionDescription:
      "활성화된 모듈의 설명과 로어북 내용이 채팅 시스템 프롬프트에 추가됩니다.",
    moduleDelete: "모듈 삭제",
    moduleStats: (lorebook, regex, trigger) =>
      `로어북 ${lorebook} · 정규식 ${regex} · 트리거 ${trigger}`,
    ...
    moduleDisabled: "비활성",
    moduleNoControls:
      "감지된 Risu 토글이 없습니다. 모듈 자체 활성화만 사용할 수 있습니다.",
    moduleSelectHint: "왼쪽에서 모듈을 가져오세요.",
    moduleControlsCount: (controls, lorebook) =>
      `토글 ${controls}개 · 로어북 ${lorebook}`,
    moduleToggleOn: "켜짐",
    moduleToggleOff: "꺼짐",
    moduleControlNames: {
  ...
  en: {
    ...
    languageGateDescription:
      "Select the language for EverTalk UI and local model responses.",
    languageKo: "한국어",
    languageEn: "English",
    languageZhCn: "简体中文",
    continue: "Start",
    ...
    familiarityDescription:
      "Messages and saved memories stored in this browser's IndexedDB are used to calculate familiarity.",
    ...
    familiarityStepHint: (step) =>
      `One star is added for every ${step} familiarity points, with no cap.`,
    ...
    memoryInsightEmpty:
      "This spirit has no memories yet. Chatting accumulates memories in the browser IndexedDB.",
    memoryInsightCount: (shown, total) => `Latest ${shown} of ${total}`,
    memoryOverviewTitle: "All Spirits Insight",
    memoryOverviewTotals: (spirits, messages, memories) =>
      `${spirits} spirits · ${messages} messages · ${memories} memories`,
    ...
    inventory: "Inventory",
    inventoryEmpty:
      "No spirit has reached level 40 yet. Grow a bond to level 40 to earn its bond sticker.",
    ...
    proactiveNotificationHint:
      "Select a Soul to open the conversation they started and mark it as read.",
    proactiveUnreadCount: (count) =>
    ...
    resetDescription: {
      sqlite:
        "Deletes every row of the local server SQLite database, the localStorage of this origin and any IndexedDB left on this origin by earlier versions, including chats, soul/style/knowledge data, memories, modules, and settings, then reloads the page.",
      indexeddb:
        "Deletes every IndexedDB database and the localStorage of this origin, including chats, soul/style/knowledge data, memories, modules, settings, and file links, then reloads the page. If another tab holds the database open, deletion stops and an error is shown.",
    },
    ...
    firstMessageHint:
      "Conversation history accumulates from the first message. On the plain web it is stored in this browser IndexedDB; through the EVAI local server it is stored in the SQLite database in the server folder.",
    ...
    personaPackChecking:
      "Checking the soul data pack and IndexedDB connection.",
    loadingBondRanking: "Loading bond ranking",
    noBondData: "No accumulated conversations",
    bondDescription:
      "Ranking is calculated from actual messages and saved memories.",
    ...
    emptyProfilePanel:
      "Select a soul to view TBL profile and asset connection status.",
    ...
    platformGuideCheckbox: {
      web_chrome:
        "I have reviewed the Chrome on-device model and browser storage described above.",
      android_app:
        "I have read the notice above and understand that this app works only with LiteRT-LM models installed on this device.",
    },
    platformGuideConfirm: "Confirm and Enter",
    platformBlockedTitle: "Unsupported Environment",
    platformBlockedMessages: {
      mobile_device:
        "Mobile web is not supported. Use a desktop browser on a PC or the dedicated Android app.",
      unsupported_browser:
        "This desktop environment could not be identified. Use a current desktop browser such as Chrome, Edge, Firefox, or Whale.",
    },
    platformBlockedHint:
      "Supported: current desktop browsers on Windows, macOS, Linux, or ChromeOS",
    messageSendFailed: "Failed to generate a response. Please try again.",
    modelListTitle: "Chat Models",
    modelListDescription: {
      web_chrome:
        "You can choose the Chrome Prompt API models (Gemini Nano · Gemma 4) and on-device models installed by Chrome. When the page is served by the EVAI local server, every model installed in Ollama (Ollama library, Hugging Face GGUF, or created from a GGUF file) appears in local Ollama mode as well, and the selected model stays fixed for chats.",
      android_app:
        "On-device AI models run on this device by the Google LiteRT-LM engine. Install and choose the model used for chat. Loading a model tries the GPU backend first and falls back to the CPU backend when the GPU cannot be used.",
    },
    modelRoleChat:
      "Chat generation · Chrome Prompt API (built-in browser model)",
    modelInputModalities: (image, audio) =>
      `Inputs · text · image ${image} · audio ${audio}`,
    modelSamplingParams: (
      defaultTopK,
      maxTopK,
      defaultTemperature,
      ...
      `Sampling · topK default ${defaultTopK} / max ${maxTopK} · temperature default ${defaultTemperature} / max ${maxTemperature}`,
    modelProbeFailed: (detail) => `Browser model probe failed: ${detail}`,
    modelSamplingParamsWebUnavailable:
      "Sampling parameters · this browser does not expose LanguageModel.params() to web pages (controlled via samplingMode)",
    chromeBuiltInAiApiNames: {
      language_model: "Prompt API (LanguageModel)",
    ...
    chromeBuiltInAiApiState: (apiName, languagePair, state) =>
      `Chrome built-in AI · ${apiName}${languagePair ? ` ${languagePair}` : ""} · ${state}`,
    chromeBuiltInAiApiNotExposed: "Not exposed in this browser",
    chromeBuiltInAiApiFailed: (detail) => `Status check failed (${detail})`,
    chromeBuiltInAiInventoryReadAt: (readAt) =>
      `Chrome built-in AI checked at · ${readAt}`,
    chromeOnDeviceInventoryUnavailable: (detail) =>
      `Could not read Chrome built-in AI status (${detail})`,
    chromePromptVariantTitle: {
      nano: "Chat · Chrome Prompt API · Gemini Nano (default)",
      gemma4: "Chat · Chrome Prompt API · Gemma 4 (flag switch)",
    },
    chromePromptVariantFlag: (flagUrl, requiresEnabled) =>
      `Required Chrome flag · ${flagUrl} = ${requiresEnabled ? "Enabled" : "Default"} (relaunch Chrome after changing)`,
    chromePromptVariantVerification: {
      active: "Chrome flag state verified · this model will run",
      flag_mismatch:
        "Chrome flag state does not match this model · change the flag, relaunch, then relink Local State",
      unverified:
        "Chrome flag state not verified · link the Local State file below to verify",
    },
    chromePromptVariantInstalled: (modelName, componentVersion, megabytes) =>
      `Installed · ${modelName} · component ${componentVersion} · ${megabytes} MB`,
    chromePromptVariantAsset: (assetId, version) =>
      `Chrome model record · ${assetId} · ${version}`,
    chromePromptVariantLastUsed: (usedAt) => `Last used in Chrome · ${usedAt}`,
    chromeBrowserModelState: (gemma4FlagEnabled, chromeVersion, readAt) =>
      `Chrome flag state · Gemma 4 ${gemma4FlagEnabled ? "Enabled" : "Default (Nano)"}${chromeVersion ? ` · Chrome ${chromeVersion}` : ""} · read ${readAt}`,
    chromeBrowserModelStateMissing:
      'Chrome flag state has not been read yet. Link it with "Select Local State file".',
    ...
    chromeInstalledModelMeta: (
      ...
      `${store} · component ${componentVersion} · ${megabytes} MB · weights ${format === "litertlm" ? "LiteRT-LM" : "Chrome-only format"}${performanceHints.length > 0 ? ` · performance hints ${performanceHints.join(", ")}` : ""}`,
    chromeInstalledModelRelinkRequired:
      "Choice saved · select the model folder again in this session to run it",
    chromeInstalledModelNotRunnable:
      "Chrome-only format, cannot run directly in this app (usable only through the Chrome Prompt API)",
    chromeInstalledModelRunnable:
      "Linked · available in on-device AI mode of the chat model selector",
    chromeInstalledModelPathLabel: "Browser user data folder path",
    chromeInstalledModelPathPlaceholder:
      "C:\\Users\\you\\AppData\\Local\\Google\\Chrome\\User Data",
    chromeInstalledModelPathHint:
      "The path differs per browser (Chrome, Edge, …). After saving, the model folder paths appear below; copy one and paste it into the folder picker address bar.",
    chromeInstalledModelStorePath: (storePath) =>
      `Model folder · ${storePath} `,
    ...
    ollamaModelSectionDescription:
      "Manage the connection state and address of the Ollama instance the EVAI local server relays to. Every model installed in Ollama appears in local Ollama mode of the chat model selector above, and conversation, memory, and persona rules apply exactly as in on-device AI mode.",
    ollamaServerConnected: (version) => `Ollama connected · version ${version}`,
    ollamaServerUnavailable:
      "Ollama is not connected · check that Ollama is running and that the Ollama address used by the EVAI local server is correct",
    ollamaModelEmpty:
      "No models are installed in Ollama. Download one with ollama pull in a terminal.",
    ollamaModelMeta: (family, parameterSize, quantization, megabytes) =>
      `${family} · ${parameterSize} · ${quantization} · ${megabytes} MB`,
    ollamaBaseUrlLabel: "Ollama address",
    ollamaBaseUrlPlaceholder: "http://127.0.0.1:11434",
    ollamaBaseUrlHint:
      "The Ollama address the EVAI local server relays to. Enter only the http protocol, host, and port without a path. The default is http://127.0.0.1:11434. The saved value is written to the server SQLite settings, and both the server relay and the server console Ollama check use only this one value.",
    ollamaBaseUrlSave: "Save address",
    generationLimitsTitle: "Context and reply length",
    generationLimitsDescription:
      "The model is loaded with exactly the values you enter here. Leave a field empty to use whatever the engine reports.",
    generationLimitsContextLabel: "Context tokens",
    generationLimitsContextHint:
      "Ollama reloads the model with this value as num_ctx. A value above the model maximum is lowered to that maximum.",
    generationLimitsOutputLabel: "Reply tokens",
    generationLimitsOutputHint:
      "Maximum tokens generated for one reply. This much of the context is reserved for the answer.",
    generationLimitsAutoPlaceholder: "Automatic",
    generationLimitsInvalid:
      "Enter a whole number of 1 or more, or leave it empty.",
    generationLimitsModelMaximum: (tokens) =>
      `Model maximum ${tokens.toLocaleString("en-US")} tokens`,
    generationLimitsActive: (tokens) =>
      `Currently applied ${tokens.toLocaleString("en-US")} tokens`,
    generationLimitsUnknownMaximum:
      "The model maximum is known once the model is loaded",
    ollamaBaseUrlSaving: "Saving…",
    ollamaGuideTitle: "Local Ollama Connection Guide",
    ollamaGuideDescription:
      'The EVAI local server relays requests to Ollama on this PC. Prepare Ollama with the commands below, run the local server, then press "Check connection". Connected models are chosen in local Ollama mode under Chat Models in setup and settings, and the chosen model stays fixed for chats.',
    ollamaConnectionChecking: "Checking the Ollama connection…",
    ollamaConnectionNotChecked:
      "The Ollama connection has not been checked yet",
    ollamaConnectionReady: (version, modelCount) =>
      modelCount > 0
    ...
    localServerNoticeDescription:
      "This page is served as a plain web page, so only Chrome on-device AI is available and data is stored in this browser IndexedDB. Download and run the EVAI local server from the repository to use local Ollama mode (Ollama library and Hugging Face GGUF models) together with the SQLite database.",
    localServerNoticeSteps: [
    ...
    localServerConnected: (version, sqliteVersion) =>
      `EVAI local server connected · version ${version} · SQLite ${sqliteVersion}`,
    localServerDatabasePath: (databasePath) =>
    ...
    ollamaCommandStepTitles: {
      ...
      expose_network:
        "(Only for Ollama on another PC) Expose Ollama to the network",
      run_local_server: "Run the EVAI local server",
    },
    ollamaCommandStepDescriptions: {
      verify_install:
        "After installing Ollama, check that a version number is printed. If the command is not found, close and reopen the terminal; if it still fails, reinstall Ollama.",
      pull_model:
        "Download the entered model from the Ollama library or Hugging Face (hf.co/…). Any model that fits your PC can be used.",
      create_from_gguf:
        "Create a model with the entered name from the entered GGUF file (or a sha256 file in Ollama blobs) as FROM.",
      run_model:
        "Run the model once to confirm it works (exit with /bye), list installed models with ollama ls, and running models with ollama ps. The app uses only the model you choose in local Ollama mode of the chat model selector.",
      remove_model: "Delete a model you no longer use.",
      expose_network:
        'Skip this when Ollama and the EVAI local server run on the same PC. Only when using Ollama running on another PC, turn on "Expose Ollama to the network" there (or set OLLAMA_HOST to 0.0.0.0:11434 and restart Ollama), then change the Ollama address under Settings > Chat Models to that PC.',
      run_local_server:
        "Run the EVAI local server from the folder that holds the web bundle and open http://127.0.0.1:9999/. The browser talks to Ollama only through this server.",
    },
    ollamaCommandCopy: "Copy commands",
    ollamaRecommendedModelTitle: (modelName) =>
      `Recommended model: ${modelName}`,
    ollamaRecommendedModelHint:
      "Download it in a terminal with the command below, then press Refresh to see it in the list.",
    ollamaGuideModelNameLabel: "Model name to use",
    ollamaGuideModelNamePlaceholder:
      "model:tag or hf.co/user/repository:quantization",
    ollamaGuideModelNameHint:
      "Download, run, and remove commands are built from the entered name. Installed models can be picked with the buttons below.",
    ollamaGuideGgufPathLabel: "GGUF file path (optional)",
    ollamaGuideGgufPathPlaceholder:
      "Absolute path of a .gguf or blobs/sha256-… file on your PC",
    ollamaGuideGgufPathHint:
      "Entering a path adds a command that creates the model above from that file.",
    modelRoleAndroidGeminiNano:
      "Chat generation · Android AICore (Gemini Nano)",
    modelAndroidGeminiNanoUnsupported:
      "This device does not support AICore Gemini Nano (requires Android 12+ and an AICore-capable device)",
    modelLanguageSupport: (languageTag, declared) =>
      declared
    ...
    localModelSections: {
      litert_lm: {
        ...
        description:
          "Runs .litertlm models from Hugging Face litert-community inside this device with the Google LiteRT-LM engine. Model files are kept only in the app's internal storage, and conversations, memories, persona rules, and the no-emoji rule apply the same way.",
    ...
    localModelGated:
      "Requires signing in to Hugging Face and accepting the model terms before downloading",
    localModelBackend: (backend) => `Backend ${backend}`,
    localModelFileMeta: (fileName, sizeMb, license) =>
      [
        fileName,
        sizeMb === null ? null : `${sizeMb}MB`,
        license === null ? null : `License ${license}`,
      ]
        .filter((part) => part !== null)
        .join(" · "),
    ...
    modelSessionDetail: (
      ...
      `${personaId} · context ${cachedTokens}/${contextWindow} · reused ${reusedTokens}`,
    modelRequestDetail: (
      state,
      promptTokens,
      generatedTokens,
      ...
      `${state} · prompt ${promptTokens ?? "-"} · generated ${generatedTokens ?? "-"} · truncated ${truncatedTokens}`,
    backupTitle: "Save · Load Data",
    backupDescription: {
      sqlite:
        "Only when you press Back up now does the server create a consistent snapshot of the SQLite database and write it into the evai-backup folder as a .sqlite3 file. There is no automatic backup, and restoring replaces the current database with the snapshot you choose.",
      indexeddb:
        "Only when you press Export does this browser's serializable IndexedDB data (chats, memories, settings, modules, and more) become a JSON file; loading validates it, replaces the data in a single transaction, and reloads the page. There is no automatic backup.",
    },
    backupStorageScope: {
      sqlite:
        "A snapshot holds the whole server SQLite database, so it restores exactly as it was. localStorage and folder permissions are excluded.",
      indexeddb:
        "JSON replaces only serializable data in this browser's IndexedDB. localStorage is excluded.",
    },
    ...
    backupFolderTitle: "Backup",
    backupFolderDescription:
      "There is no automatic backup. A backup file is created only when you press Back up now, and you can restore any point from the list exactly as it was. The 10 most recent are kept.",
    backupFolderLinked: (name) => `Backup location: ${name}`,
    backupFolderNotLinked: "Backup is unavailable.",
    backupFolderLink: "Open backup",
    ...
    backupLastError: (message) => `Backup failed: ${message}`,
    backupFilesEmpty: "No backup files yet.",
    backupFileMeta: (dateTime, sizeKb) => `${dateTime} · ${sizeKb}KB`,
    backupFileRestore: "Restore",
    backupRestoreConfirm: (fileName) =>
      `Restoring ${fileName} replaces all current data. Continue?`,
    backupWritten: (fileName) => `Saved backup ${fileName}.`,
    resetStorageScope: {
      sqlite:
        "Resets the local server SQLite database and this origin's localStorage, and deletes any IndexedDB left on this origin.",
      indexeddb:
        "Resets this origin's localStorage and every IndexedDB database.",
    },
    navChat: "Chat",
    ...
    navStory: "Story",
    navStorage: "Storage",
    storyMainTitle: "Main Story",
    storyLoveTitle: "Bond Story",
    storyChapterLabel: "Chapter",
    storyEpisodeLabel: "Episode",
    storyEndingBad: "Bad Ending",
    storyEndingNormal: "Normal Ending",
    storyEndingTrue: "True Ending",
    storyRequiredAffinity: "Required affinity",
    storyPrevious: "Previous",
    storyNext: "Next",
    storyBackToList: "Back to list",
    storyChoicePrompt: "Savior's choice",
    storyLoading: "Loading the story...",
    storyLoadFailed: "Could not load the story.",
    storyEmpty: "No story to show.",
    ...
    bgmPlay: "Play",
    bgmPause: "Pause",
    bgmPrevious: "Previous track",
    bgmNext: "Next track",
    bgmOrderListed: "Listed order",
    bgmOrderTitle: "By title",
    bgmOrderShuffle: "Shuffle",
    bgmSearch: "Search tracks",
    bgmEmpty: "No tracks",
    bgmVolume: "Volume",
    storyLogEmpty: "No lines read yet.",
    navCheat: "Cheat Mode",
    navGuide: "Guide",
    navSetup: "Setup",
    ...
    guidePageDescription:
      "A step-by-step walkthrough, matched to the current state of your PC, from what an AI model is to your first conversation with a spirit.",
    guideBeginnerTitle: "New here? Start with this",
    guideBeginnerIntro:
      'EverTalk writes each spirit\'s replies with an AI that runs inside your own PC, not an online AI service. Your conversations never leave the PC, but you need to prepare the AI model that writes the replies. Read the terms below once, then follow "Follow along with your current setup" from the top.',
    guideConcepts: [
      {
        term: "AI model (LLM)",
        description:
          "An artificial-intelligence file that reads text and writes a reply. EverTalk organizes each spirit's personality, voice and memories and hands them to the model, which then writes the spirit's reply.",
      },
      {
        term: "Local LLM",
        description:
          "An AI model that runs on your own PC's graphics card (GPU) and memory instead of a company server. There is no usage fee and chats stay on the PC, but reply speed and usable model size depend on your hardware.",
      },
      {
        term: "Chrome on-device AI",
        description:
          "The AI built into desktop Chrome (Gemini Nano by default, Gemma 4 when its flag is on). Nothing extra to install; Chrome downloads the model once. The easiest way to start.",
      },
      {
        term: "Ollama",
        description:
          "A free program that downloads and runs local LLMs. Once installed it stays running in the background, and one command downloads the model you want. It lets you use larger, more expressive models than Chrome's built-in one.",
      },
      {
        term: "Model names and sizes",
        description:
          'Ollama models are named "name:tag". Numbers such as 4b or 9b are the model size (4 or 9 billion parameters): larger is smarter but needs more memory. Marks such as q4 or Q4_K_M mean a compressed version. Start with a model whose file is smaller than your graphics memory (VRAM).',
      },
      {
        term: "Hugging Face · GGUF",
        description:
      ...
      {
        term: "EVAI local server",
        description:
          "A small program (evai-server) that serves the EverTalk screen from your PC. Opening the app through it lets you use Ollama models and stores chats, memories and settings in the SQLite database in the server folder. Opened from a plain web address, only Chrome on-device AI is available and data stays in this browser (IndexedDB).",
      },
    ],
    ...
    guideChecklistIntro: {
      ...
      local_server:
        'This page is open through the EVAI local server, so Ollama models are available (Chrome on-device AI works as well). Every completion mark is read from the actual state of this PC; press "Refresh status" after finishing a step to check again.',
    },
    guideStepTitles: {
      use_pc_chrome: "Open in desktop Chrome",
      prepare_on_device: "Prepare Chrome's built-in AI model",
      get_local_server:
        "(Optional) Run the EVAI local server to use Ollama models",
    ...
    guideStepDescriptions: {
      use_pc_chrome:
        'On-device AI works only in current desktop Chrome on Windows, macOS or Linux. It needs at least 22 GB of free storage and more than 4 GB of graphics memory (or 16 GB of RAM). If this does not turn complete, update Chrome and press "Refresh status".',
      prepare_on_device:
        'Under "Open settings" > Chat Models, press "Download and prepare" on Gemini Nano and Chrome downloads the model. It is large and can take a few minutes; the step is complete when the state becomes "Available".',
      get_local_server:
        'Download evai-server from "Open repository", run it, and open http://127.0.0.1:9999/ in the browser; this guide then switches to the Ollama installation steps.',
      run_local_server:
        "This screen is already served by the EVAI local server, so this step is complete. Closing the server window stops the app, so keep it open while you use it.",
      install_ollama:
        'Get the installer from the official site with "Download Ollama" and install it. Ollama starts automatically afterwards (on Windows its icon appears in the notification area at the right of the taskbar). Then press "Refresh status" to check the connection.',
      pull_model:
        'Open a terminal as described in "How to enter commands" below and type ollama pull model-name. Pick a model from "Browse Ollama models" or "Hugging Face GGUF guide"; entering it in "Model name" of the connection guide below builds the command for you. Press "Refresh status" when the download finishes.',
      select_model:
        "In the setup screen or Settings > Chat Models, pick on-device AI mode or local Ollama mode and select one model. The choice is saved and applied identically on every screen.",
      start_chat:
        'Once the model shows "Running" you are ready. Pick a spirit in the lobby or chat screen and say hello. The first reply can take a little longer while the model loads.',
    },
    guideStepStates: {
      checking: "Checking",
      done: "Done",
    ...
    chatModelSelectorDescription:
      "Choose between on-device AI mode and local Ollama mode, then pick a model that is actually connected in that mode. Setup and settings share the same list and the same saved value.",
    chatModelModeTitles: {
      on_device: "On-device AI mode",
      ollama: "Local Ollama mode",
    },
    chatModelRuntimeStates: {
      checking: "Checking",
      running: "Running",
    ...
    chatModelModeEmpty: {
      on_device:
        "No on-device model can be selected in this browser right now. Prepare a Chrome on-device model under Settings > Chat Models, or open the guide.",
      ollama:
        "No Ollama model can be selected. Start Ollama and pull a model with the commands in the guide, then refresh.",
    },
    chatModelActiveMode: "Current chat mode",
    chatModelOptionCount: (count) => `${count} selectable`,
    chatModelOllamaLocalServerOnly:
      "Local Ollama mode is available only when the app is opened through the EVAI local server. The guide explains how to run it.",
    chatModelSavedTo: (storageName) =>
      `The selected model is saved in the ${storageName} and applied identically on every screen.`,
    spiritActionNote: (spiritName, actions) =>
      `${spiritName}'s action: ${actions}`,
    proactiveMessages: "Spirits message you first",
    proactiveMessagesDescription:
      "When on, the spirit you are chatting with and your other spirits send you messages on their own, driven by the longing, affection and jealousy built up in your conversations and by their personality. Forward spirits may send several before you reply.",
    cheatMode: "Cheat mode",
    cheatModeDescription:
      "Adds a Cheat Mode view to the top bar and applies each spirit's bond level, personality, emotion, and speech presets to real replies.",
    cheatPageTitle: "Spirit Cheat Settings",
    cheatPageDescription:
      "Set each spirit's bond level directly and switch their base personality, emotion, and way of speaking with presets. Changes apply from the next reply.",
    ...
    cheatBondDescription:
      "Locks the relationship stage and bond display to this level regardless of conversation history.",
    ...
    cheatEmotionDescription:
      "Selecting a preset switches the current emotion immediately, and later mood changes settle back toward it.",
    cheatSpeechTitle: "Way of speaking",
    cheatReset: "Reset this spirit's cheats",
    storagePageTitle: "Conversation Database Analytics",
    storagePageDescription:
      "Inspect the active mode, actual storage location, store and spirit record volume, capacity composition, and recent saved content.",
    refreshAnalysis: "Refresh analysis",
    storageModeActive: "Active storage mode",
    browserManagedLocation: "Browser-managed storage",
    browserManagedLocationDetail:
      "Browser security does not expose the exact OS path of IndexedDB to the web. It is identified by the origin and logical database name below.",
    ...
    setupModelSelectionHint:
      "Choose the chat mode and model now. What you pick here is the same saved value as Settings > Chat Models.",
    setupModelRequired: "Select one model to start.",
    setupModelLoading:
      "You can start once the selected model is fully loaded and running.",
    setupModelLoadFailed: (detail) =>
      `The selected model could not be loaded, so setup cannot start. Select the model again or refresh the list. (${detail})`,
    storageObjectKinds: {
    ...
    rankingPageDescription:
      "Compare every spirit using real bond scores calculated from accumulated conversations and memories.",
    bondScoreLabel: "Bond score",
    memoryPageTitle: "Memory Algorithm Flow",
    memoryPageDescription:
      "The live flow that analyzes every stored conversation for keyword priority, relationships with other spirits and inner state to build the behavior steps of the next reply.",
    memoryGraphConnections: "connections",
    memoryContextKinds: {
      conversation: "DB conversation flow",
      directive: "User directive",
      episodic: "Conversation event",
      habit: "Keyword threads",
      affect: "Emotion & jealousy",
      knowledge: "World knowledge",
    },
    memoryFilterTitle: "Memories used in replies",
    memoryFilterDescription:
      "Kinds you turn off are left out of the spirit's next reply context.",
    memoryFilterSearchPlaceholder: "Search keywords",
    memoryFilterEmpty:
      "No keywords to show yet. Talk with the spirit and the keyword graph will grow.",
    memoryGraphRecentOnly: "Only keywords in the recent conversation",
    memoryGraphNoSpirit:
      "Select a spirit to analyze that spirit's memory graph.",
    memoryGraphSelectHint:
      "Scroll to zoom, drag empty space to pan, drag nodes to arrange. Select a keyword or spirit to see the conversations and what the spirit did at the time.",
    memoryNetworkViewAll: "All relationships",
    memoryNetworkViewAllDescription:
      "See every spirit with history on one screen, including the jealousy they hold toward each other.",
    memoryNetworkOnlySpirit: "One spirit",
    memoryNetworkEmpty:
      "There is no history to draw a network yet. Chat with spirits to fill it in.",
    memoryNetworkNodeValue: (level, affection) =>
      `Lv.${level} · affection ${affection}`,
    ...
    memorySpiritRosterMeta: (level, messageCount) =>
      `Lv.${level} · ${messageCount} messages`,
    memoryGraphLegend: {
      query: "Linked to the latest input or mention",
      recent: "Recent conversation",
      history: "Accumulated memory",
    },
    memoryGraphLegendTitle: "Graph legend",
    memoryGraphKeywordCounts: (userCount, spiritCount) =>
      `Savior ${userCount} · Spirit ${spiritCount}`,
    memoryGraphSaviorValue: (messageCount) => `${messageCount} messages shared`,
    memoryGraphRelationValue: (saviorLevel) =>
      saviorLevel === null
        ? "No bond with the Savior"
        : `Savior bond Lv.${saviorLevel}`,
    memoryGraphEdgeKinds: {
      ...
      rival_attention:
        "Savior talked with another spirit since the last chat (jealousy)",
    ...
    memoryGraphEdgeSaviorBondLabel: (level, messageCount) =>
      `bond Lv.${level} · ${messageCount} messages`,
    memoryGraphEdgeCanonBondLabel: (strength, sharedUnion) =>
      sharedUnion === null
        ? `canon bond ${strength}`
        : `same group ${sharedUnion} · bond ${strength}`,
    memoryGraphEdgeRelationSaviorLabel: (level) => `Savior bond Lv.${level}`,
    memoryGraphEdgeRivalLabel: (messageCount) =>
      `jealousy · ${messageCount} messages`,
    memoryGraphEdgeJealousyLabel: (
      firstName,
      firstToSecondStir,
      secondName,
    ...
    memoryGraphPersonaHeartValue: (level, affection) =>
      affection === null
        ...
        : `Lv.${level} · affection ${affection}`,
    memoryGraphJealousyNodeValue: (strongestStir) =>
      `jealousy up to ${strongestStir}%`,
    memoryHeartTitle: "Emotional swings",
    memoryHeartDescription: (spiritName) =>
      `Inner state of ${spiritName}, accumulated day by day from your stored conversations. It drives who messages first and who gets jealous.`,
    memoryHeartMetrics: {
      affection: "Affection",
      trust: "Trust",
      longing: "Longing",
    ...
    memoryHeartExpressions: {
      openness: "Openness",
      outward_warmth: "Warmth shown outwardly",
      receptiveness: "Receptiveness",
      initiative: "Initiative",
    },
    memoryHeartChartLabel: (spiritName) =>
      `Daily affection, trust, longing and hurt of ${spiritName}`,
    memoryHeartRivalBarLabel: "Savior messages to other spirits",
    memoryHeartRivalCount: (count) => `${count} messages to other spirits`,
    memoryHeartContactSummary: (contactDays, saviorMessages) =>
      `${contactDays} days talked · ${saviorMessages} Savior messages`,
    memoryHeartEmpty:
      "No conversation yet, so there are no emotional changes to chart.",
    ...
    memoryRelationCanonStats: (interactionCount, mentionCount) =>
      `${interactionCount} canon interactions · ${mentionCount} mentions`,
    memoryRelationSaviorBond: (level, messageCount) =>
      `Bond with the Savior Lv.${level} · ${messageCount} messages`,
    memoryRelationNoSaviorBond: "No conversation with the Savior yet.",
    memoryKeywordDetailTitle: (token) => `Keyword · ${token}`,
    memoryKeywordStats: (userCount, spiritCount, firstSeen, lastSeen) =>
      `Savior ${userCount}× · Spirit ${spiritCount}× · first ${firstSeen} · latest ${lastSeen}`,
    memoryKeywordRecent: (recentCount) =>
      `${recentCount}× in the recent conversation`,
    ...
    memoryRivalStats: (userCount, spiritCount, firstAt, latestAt) =>
      `${userCount} messages from the Savior · ${spiritCount} replies · ${firstAt} – ${latestAt}`,
    memoryRivalNoContact:
      "No conversation with this spirit since the last chat.",
    memoryRivalTopics: "Topics",
    memoryRivalSpokeOfYou: (count) =>
      `This spirit was mentioned ${count} times there`,
    memoryRivalMentionedNow: "Mentioned in the latest input",
    memoryRivalCanonBond: (addressForm) =>
      `Canon form of address: ${addressForm}`,
    memoryRivalSharedUnion: (union) => `Same group: ${union}`,
    memoryBehaviorStages: {
      input: {
        title: "Savior's input",
        description: "Words, actions and descriptions just received",
      },
      keywords: {
        title: "Keyword threads",
        description: "Priority keywords linked to the input and recent context",
      },
      recall: {
        title: "Memory recall",
        description: "Consolidated memory and related events in time order",
      },
      social: {
        title: "Other spirits",
        description: "Conversations that drive jealousy and rivalry",
      },
      inner_state: {
        title: "Inner state",
        description: "The spirit's own heart and next intention",
      },
    ...
    modulesSectionDescription:
      "The description and lorebook of enabled modules are added to the chat system prompt.",
    moduleDelete: "Delete module",
    moduleStats: (lorebook, regex, trigger) =>
      `Lorebook ${lorebook} · Regex ${regex} · Trigger ${trigger}`,
    ...
    moduleNoControls:
      "No Risu toggles detected. Only module-level enabling is available.",
    moduleSelectHint: "Import a module on the left.",
    moduleControlsCount: (controls, lorebook) =>
      `${controls} toggles · Lorebook ${lorebook}`,
    moduleToggleOn: "On",
    moduleToggleOff: "Off",
    moduleControlNames: {
  ...
  zh_cn: {
    ...
    familiarityDescription:
      "将根据本浏览器 IndexedDB 中累积的消息与记忆计算亲密度。",
    ...
    inventory: "仓库",
    inventoryEmpty:
      "还没有精灵达到40级。与精灵对话把羁绊提升到40级即可获得羁绊贴纸。",
    ...
    resetDescription: {
      sqlite:
        "删除本地服务器 SQLite 数据库的全部数据、当前来源的 localStorage 以及旧版本在当前来源留下的 IndexedDB，包括聊天、精灵/风格/知识数据、记忆、模块与设置，然后重新载入页面。",
      indexeddb:
        "删除当前来源的全部 IndexedDB 数据库与 localStorage，包括聊天、精灵/风格/知识数据、记忆、模块、设置及文件连接，然后重新载入页面。如果其他标签页仍占用数据库，删除会中止并显示错误。",
    },
    resetFailed: "重置失败",
    notConfigured: "未设置",
    resetting: "正在重置...",
    ...
    firstMessageHint:
      "从第一条消息起开始累积对话。普通网页保存在本浏览器的 IndexedDB 中，通过 EVAI 本地服务器时保存在服务器文件夹的 SQLite 数据库中。",
    ...
    platformGuideCheckbox: {
      ...
      android_app:
        "我已阅读以上说明，并了解本应用仅能使用安装在本设备上的 LiteRT-LM 模型运行。",
    },
    platformGuideConfirm: "确认并进入",
    platformBlockedTitle: "不支持的使用环境",
    platformBlockedMessages: {
      mobile_device:
        "不支持移动网页。请使用电脑桌面浏览器或专用 Android 应用。",
      unsupported_browser:
        "无法识别此桌面环境。请使用 Chrome、Edge、Firefox、Whale 等最新版桌面浏览器。",
    },
    platformBlockedHint:
      "支持环境：Windows、macOS、Linux、ChromeOS 上的最新版桌面浏览器",
    messageSendFailed: "生成响应失败。请重试。",
    modelListTitle: "对话模型",
    modelListDescription: {
      web_chrome:
        "可以选择 Chrome Prompt API 模型（Gemini Nano · Gemma 4）与 Chrome 安装的设备端模型。通过 EVAI 本地服务器打开时，Ollama 中安装的所有模型（Ollama 模型库、Hugging Face GGUF、由 GGUF 文件创建）会一并显示在本地 Ollama 模式中，所选模型会固定用于对话。",
      android_app:
        "这是在本设备上由 Google LiteRT-LM 引擎运行的设备端 AI 模型。请安装并选择用于对话的模型。加载模型时会优先尝试 GPU 后端，无法使用时改用 CPU 后端运行。",
    },
    modelRoleChat: "对话生成 · Chrome Prompt API（浏览器内置模型）",
    modelInputModalities: (image, audio) =>
      `输入支持 · 文本 · 图像 ${image} · 音频 ${audio}`,
    modelSamplingParams: (
      defaultTopK,
      maxTopK,
      defaultTemperature,
      ...
      `采样 · topK 默认 ${defaultTopK} / 最大 ${maxTopK} · 温度 默认 ${defaultTemperature} / 最大 ${maxTemperature}`,
    modelProbeFailed: (detail) => `浏览器模型信息查询失败：${detail}`,
    modelSamplingParamsWebUnavailable:
      "采样参数 · 此浏览器未向网页提供 LanguageModel.params()（通过 samplingMode 控制）",
    chromeBuiltInAiApiNames: {
      language_model: "Prompt API（LanguageModel）",
    ...
    chromeBuiltInAiApiState: (apiName, languagePair, state) =>
      `Chrome 内置 AI · ${apiName}${languagePair ? ` ${languagePair}` : ""} · ${state}`,
    chromeBuiltInAiApiNotExposed: "此浏览器未提供",
    chromeBuiltInAiApiFailed: (detail) => `状态检查失败（${detail}）`,
    chromeBuiltInAiInventoryReadAt: (readAt) =>
      `Chrome 内置 AI 检查时间 · ${readAt}`,
    chromeOnDeviceInventoryUnavailable: (detail) =>
      `无法读取 Chrome 内置 AI 状态（${detail}）`,
    chromePromptVariantTitle: {
      nano: "对话生成 · Chrome Prompt API · Gemini Nano（默认）",
      gemma4: "对话生成 · Chrome Prompt API · Gemma 4（旗标切换）",
    },
    chromePromptVariantFlag: (flagUrl, requiresEnabled) =>
      `所需 Chrome 旗标 · ${flagUrl} = ${requiresEnabled ? "Enabled" : "Default"}（更改后重启 Chrome）`,
    chromePromptVariantVerification: {
      active: "Chrome 旗标状态已确认 · 将使用此模型运行",
      flag_mismatch:
        "Chrome 旗标状态与此模型不符 · 请更改旗标并重启后重新连接 Local State",
      unverified: "Chrome 旗标状态未确认 · 在下方连接 Local State 文件即可验证",
    },
    chromePromptVariantInstalled: (modelName, componentVersion, megabytes) =>
      `已安装 · ${modelName} · 组件 ${componentVersion} · ${megabytes} MB`,
    chromePromptVariantAsset: (assetId, version) =>
      `Chrome 模型记录 · ${assetId} · ${version}`,
    chromePromptVariantLastUsed: (usedAt) => `Chrome 中最后使用 · ${usedAt}`,
    chromeBrowserModelState: (gemma4FlagEnabled, chromeVersion, readAt) =>
      `Chrome 旗标状态 · Gemma 4 ${gemma4FlagEnabled ? "已启用（Enabled）" : "默认（Nano）"}${chromeVersion ? ` · Chrome ${chromeVersion}` : ""} · 读取于 ${readAt}`,
    chromeBrowserModelStateMissing:
      "尚未读取 Chrome 旗标状态。请通过“选择 Local State 文件”连接。",
    ...
    chromeInstalledModelSectionDescription:
      "Chrome 设备端 AI 默认使用 Gemini Nano，启用 gemma4-for-built-in-ai 旗标后切换为 Gemma 4。连接模型文件夹和 Local State 后会显示已安装模型和当前旗标状态，上方对话模型选择中的 Prompt API 模型会按实际旗标验证。已连接的 LiteRT-LM 格式 Gemma 会出现在上方选择器的设备端 AI 模式中，选择后本应用会直接运行其权重。",
    chromeInstalledModelEmpty:
      "尚未连接模型文件夹。请在下方保存路径并选择模型文件夹。",
    chromeInstalledModelTitle: (modelName, modelVersion) =>
      `${modelName}（${modelVersion}）`,
    ...
    chromeInstalledModelMeta: (
      ...
      `${store} · 组件 ${componentVersion} · ${megabytes} MB · 权重 ${format === "litertlm" ? "LiteRT-LM" : "Chrome 专用格式"}${performanceHints.length > 0 ? ` · 性能提示 ${performanceHints.join(", ")}` : ""}`,
    chromeInstalledModelRelinkRequired:
      "选择已保存 · 本次会话需重新选择模型文件夹才能运行",
    chromeInstalledModelNotRunnable:
      "Chrome 专用格式，无法在本应用中直接运行（只能通过 Chrome Prompt API 使用）",
    chromeInstalledModelRunnable:
      "已连接 · 可在对话模型选择的设备端 AI 模式中选择",
    chromeInstalledModelPathLabel: "浏览器用户数据文件夹路径",
    chromeInstalledModelPathPlaceholder:
      "C:\\Users\\用户\\AppData\\Local\\Google\\Chrome\\User Data",
    chromeInstalledModelPathHint:
      "不同浏览器（Chrome、Edge 等）路径不同。保存后下方会显示模型文件夹路径，复制后粘贴到文件夹选择窗口的地址栏即可。",
    ...
    ollamaModelSectionDescription:
      "管理 EVAI 本地服务器所转发的 Ollama 的连接状态与地址。Ollama 中安装的所有模型会出现在上方对话模型选择的本地 Ollama 模式中，对话、记忆与角色设定规则与设备端 AI 模式完全相同。",
    ollamaServerConnected: (version) => `Ollama 已连接 · 版本 ${version}`,
    ollamaServerUnavailable:
      "未连接 Ollama · 请确认 Ollama 正在运行，以及 EVAI 本地服务器使用的 Ollama 地址是否正确",
    ollamaModelEmpty:
      "Ollama 中没有已安装的模型。请在终端中使用 ollama pull 下载模型。",
    ollamaModelMeta: (family, parameterSize, quantization, megabytes) =>
      `${family} · ${parameterSize} · ${quantization} · ${megabytes} MB`,
    ollamaBaseUrlLabel: "Ollama 地址",
    ollamaBaseUrlPlaceholder: "http://127.0.0.1:11434",
    ollamaBaseUrlHint:
      "EVAI 本地服务器要转发到的 Ollama 地址。只输入 http 协议、主机和端口，不含路径。默认值为 http://127.0.0.1:11434。保存的值写入服务器的 SQLite 设置，服务器转发与服务器控制台的 Ollama 连接检查都只使用这一个值。",
    ollamaBaseUrlSave: "保存地址",
    generationLimitsTitle: "上下文与回复长度",
    generationLimitsDescription:
      "按这里填写的数值加载并使用模型。留空则采用引擎报告的数值。",
    generationLimitsContextLabel: "上下文令牌",
    generationLimitsContextHint:
      "Ollama 会以该值作为 num_ctx 重新加载模型。超过模型上限时按上限取值。",
    generationLimitsOutputLabel: "回复令牌",
    generationLimitsOutputHint:
      "单次回复生成的最大令牌数。上下文中会为回复预留这一部分。",
    generationLimitsAutoPlaceholder: "自动",
    generationLimitsInvalid: "请填写 1 以上的整数，或留空。",
    generationLimitsModelMaximum: (tokens) =>
      `模型上限 ${tokens.toLocaleString("zh-CN")} 令牌`,
    generationLimitsActive: (tokens) =>
      `当前生效 ${tokens.toLocaleString("zh-CN")} 令牌`,
    generationLimitsUnknownMaximum: "加载模型后才能确认模型上限",
    ollamaBaseUrlSaving: "正在保存…",
    ollamaGuideTitle: "本地 Ollama 连接指南",
    ollamaGuideDescription:
      "EVAI 本地服务器会把请求转发到本机的 Ollama。请按下面的命令准备 Ollama 并运行本地服务器，然后点击“检查连接”。已连接的模型请在初始设置与设置的对话模型中以本地 Ollama 模式选择，所选模型会固定用于对话。",
    ...
    localServerNoticeDescription:
      "当前以普通网页方式打开，只能使用 Chrome 设备端 AI，数据保存在本浏览器的 IndexedDB 中。从仓库下载并运行 EVAI 本地服务器后，可以同时使用本地 Ollama 模式（Ollama 模型库与 Hugging Face GGUF 模型）与 SQLite 数据库。",
    localServerNoticeSteps: [
    ...
    localServerConnected: (version, sqliteVersion) =>
      `EVAI 本地服务器已连接 · 版本 ${version} · SQLite ${sqliteVersion}`,
    ...
    ollamaCommandStepDescriptions: {
      verify_install:
        "安装 Ollama 后确认能输出版本号。如果找不到命令，请关闭并重新打开终端；仍然不行时请重新安装 Ollama。",
      pull_model:
        "从 Ollama 模型库或 Hugging Face（hf.co/…）下载输入的模型。可以使用任何适合电脑配置的模型。",
      create_from_gguf:
        "以输入的 GGUF 文件（或 Ollama blobs 中的 sha256 文件）作为 FROM，创建输入名称的模型。",
      run_model:
        "运行一次模型确认可用（用 /bye 退出），用 ollama ls 查看已安装模型，用 ollama ps 查看正在运行的模型。应用只使用在对话模型选择的本地 Ollama 模式中所选的模型。",
      remove_model: "删除不再使用的模型。",
      expose_network:
        'Ollama 与 EVAI 本地服务器在同一台电脑上时不需要此步骤。只有使用其他电脑上运行的 Ollama 时，才在那台电脑的 Ollama 设置中开启 "Expose Ollama to the network"（或将 OLLAMA_HOST 设为 0.0.0.0:11434 后重启 Ollama），再把 设置 > 对话模型 中的 Ollama 地址改为那台电脑的地址。',
      run_local_server:
        "在存放网页包的文件夹中运行 EVAI 本地服务器，并打开 http://127.0.0.1:9999/。浏览器只通过该服务器与 Ollama 通信。",
    },
    ollamaCommandCopy: "复制命令",
    ollamaRecommendedModelTitle: (modelName) => `推荐模型：${modelName}`,
    ollamaRecommendedModelHint:
      "在终端中用下面的命令下载模型，然后点击刷新即可在列表中看到。",
    ollamaGuideModelNameLabel: "要使用的模型名称",
    ollamaGuideModelNamePlaceholder: "模型名:标签 或 hf.co/用户/仓库:量化",
    ollamaGuideModelNameHint:
      "会根据输入的名称生成下载、运行和删除命令。已安装的模型可以通过下方按钮选择。",
    ollamaGuideGgufPathLabel: "GGUF 文件路径（可选）",
    ollamaGuideGgufPathPlaceholder:
      "电脑中 .gguf 或 blobs/sha256-… 文件的绝对路径",
    ollamaGuideGgufPathHint: "输入路径后会添加用该文件创建上述名称模型的命令。",
    modelRoleAndroidGeminiNano: "对话生成 · Android AICore (Gemini Nano)",
    modelAndroidGeminiNanoUnsupported:
      "此设备不支持 AICore Gemini Nano（需要 Android 12 及以上且支持 AICore 的设备）",
    modelLanguageSupport: (languageTag, declared) =>
      declared
    ...
    localModelSections: {
      litert_lm: {
        ...
        description:
          "通过 Google LiteRT-LM 引擎在本设备内运行 Hugging Face litert-community 的 .litertlm 模型。模型文件只保存在应用内部存储中，对话、记忆、角色设定和禁用表情符号的规则同样适用。",
    ...
    modelSessionDetail: (
      ...
      `${personaId} · 上下文 ${cachedTokens}/${contextWindow} · 复用 ${reusedTokens}`,
    modelRequestDetail: (
      state,
      promptTokens,
      generatedTokens,
      ...
      `${state} · 输入 ${promptTokens ?? "-"} · 生成 ${generatedTokens ?? "-"} · 截断 ${truncatedTokens}`,
    backupTitle: "数据保存 · 载入",
    backupDescription: {
      sqlite:
        "只有在你按下“立即备份”时，服务器才会生成 SQLite 数据库的一致快照，并以 .sqlite3 文件写入 evai-backup 文件夹。没有自动备份，恢复时会用所选快照原样替换当前数据库。",
      indexeddb:
        "只有在你按下导出时，本浏览器 IndexedDB 中可序列化的数据（聊天、记忆、设置、模块等）才会写成 JSON 文件；导入时先验证，再以单个事务替换数据并重新载入页面。没有自动备份。",
    },
    backupStorageScope: {
      sqlite:
        "快照包含整个服务器 SQLite 数据库，因此可以原样恢复。localStorage 与文件夹权限不包含在内。",
      indexeddb:
        "JSON 只替换恢复当前浏览器 IndexedDB 中可序列化的数据。localStorage 不包含在内。",
    },
    ...
    backupFolderTitle: "备份",
    backupFolderDescription:
      "没有自动备份。只有按下“立即备份”时才会创建备份文件，并可从列表选择任意时间点原样恢复。保留最近 10 个。",
    backupFolderLinked: (name) => `备份位置：${name}`,
    backupFolderNotLinked: "无法使用备份。",
    backupFolderLink: "打开备份",
    ...
    backupLastError: (message) => `备份失败：${message}`,
    backupFilesEmpty: "还没有备份文件。",
    backupFileMeta: (dateTime, sizeKb) => `${dateTime} · ${sizeKb}KB`,
    backupFileRestore: "恢复",
    backupRestoreConfirm: (fileName) =>
      `使用 ${fileName} 恢复会替换当前全部数据。是否继续？`,
    backupWritten: (fileName) => `已保存备份 ${fileName}。`,
    resetStorageScope: {
      sqlite:
        "重置本地服务器 SQLite 数据库与当前来源的 localStorage，并删除当前来源残留的 IndexedDB。",
    ...
    storyMainTitle: "主线剧情",
    storyLoveTitle: "羁绊剧情",
    storyChapterLabel: "章节",
    storyEpisodeLabel: "剧集",
    storyEndingBad: "坏结局",
    storyEndingNormal: "普通结局",
    storyEndingTrue: "真结局",
    storyRequiredAffinity: "所需好感度",
    storyPrevious: "上一句",
    storyNext: "下一句",
    storyBackToList: "返回列表",
    storyChoicePrompt: "救援者的选择",
    storyLoading: "正在加载剧情…",
    storyLoadFailed: "无法加载剧情。",
    storyEmpty: "没有可显示的剧情。",
    storyProgress: "进度",
    storyAuto: "自动",
    storyLog: "记录",
    ...
    guidePageTitle: "连接指南",
    guidePageDescription:
      "从什么是 AI 模型，到与精灵的第一次对话，按照这台电脑的当前状态一步步引导。",
    guideBeginnerTitle: "第一次使用？请从这里开始",
    guideBeginnerIntro:
      "EverTalk 使用在你电脑内运行的 AI（而不是网上的 AI 服务）来生成精灵的回复。对话内容不会发送到外部，但需要先在电脑上准备生成回复的 AI 模型。请先读一遍下面的术语，再从上到下依次完成“按当前状态一步步操作”中的步骤。",
    guideConcepts: [
      {
        term: "AI 模型（LLM）",
        description:
          "读取文字并续写回复的人工智能文件。精灵的性格、语气和记忆由 EverTalk 整理后交给模型，模型再据此写出精灵的回复。",
      },
      {
        term: "本地 LLM",
        description:
          "不在公司服务器上，而是用你电脑的显卡（GPU）和内存直接运行的 AI 模型。没有使用费，对话也不会离开电脑，但回复速度和可用的模型大小取决于电脑性能。",
      },
      {
        term: "Chrome 设备端 AI",
        description:
          "电脑版 Chrome 内置的 AI（默认 Gemini Nano，开启旗标后为 Gemma 4）。无需另外安装程序，Chrome 会在第一次使用时下载模型。这是最简单的开始方式。",
      },
      {
        term: "Ollama",
        description:
          "下载并运行本地 LLM 的免费程序。安装后会在后台运行，一条命令即可下载想要的模型。可以使用比 Chrome 内置模型更大、表现力更好的模型。",
      },
      {
        term: "模型名称与大小",
        description:
          "Ollama 模型以“名称:标签”的形式表示。标签里的 4b、9b 等数字是模型规模（40 亿、90 亿个参数），越大越聪明但占用内存越多。q4、Q4_K_M 等标记表示压缩后的版本。刚开始请选择文件大小小于显卡显存（VRAM）的模型。",
      },
      {
        term: "Hugging Face · GGUF",
        description:
          "Hugging Face 是发布 AI 模型的网站，GGUF 是本地 LLM 使用的模型文件格式。Hugging Face 上的 GGUF 模型可以用 ollama pull hf.co/用户/仓库 直接下载到 Ollama，手头已有的 GGUF 文件也可以制作成 Ollama 模型。",
      },
      {
        term: "EVAI 本地服务器",
        description:
          "在你电脑上提供 EverTalk 页面的小程序（evai-server）。通过它打开时才能使用 Ollama 模型，对话、记忆和设置会保存在服务器文件夹的 SQLite 数据库中。用普通网址打开时只能使用 Chrome 设备端 AI，数据保存在本浏览器（IndexedDB）中。",
      },
    ],
    ...
    guideChecklistIntro: {
      ...
      local_server:
        "当前通过 EVAI 本地服务器打开，可以使用 Ollama 模型（也可以同时使用 Chrome 设备端 AI）。每个完成标记都读取这台电脑的实际状态，完成步骤后点击“刷新状态”即可重新确认。",
    },
    guideStepTitles: {
    ...
    guideStepDescriptions: {
      use_pc_chrome:
        "设备端 AI 只能在 Windows、macOS、Linux 的最新电脑版 Chrome 中运行。需要至少 22GB 可用存储空间，以及超过 4GB 的显存（或 16GB 以上内存）。如果没有变为完成，请把 Chrome 更新到最新版本后点击“刷新状态”。",
      prepare_on_device:
        "在“打开设置”> 对话模型中，点击 Gemini Nano 项的“下载并准备”，Chrome 会下载模型。模型较大，可能需要几分钟；状态变为“可用”即完成。",
      get_local_server:
        "通过“打开仓库”下载并运行 evai-server，然后在浏览器中打开 http://127.0.0.1:9999/，本指南会切换为 Ollama 安装步骤。",
      run_local_server:
        "当前页面已由 EVAI 本地服务器提供，因此此步骤已完成。关闭服务器窗口会让应用停止，使用期间请保持开启。",
      install_ollama:
        "点击“下载 Ollama”从官方网站获取安装程序并安装。安装完成后 Ollama 会自动启动（Windows 任务栏右侧通知区域会出现 Ollama 图标）。然后点击“刷新状态”确认连接。",
      pull_model:
        "按照下方“如何输入命令”打开终端，输入 ollama pull 模型名称。模型可以在“浏览 Ollama 模型”或“Hugging Face GGUF 指南”中挑选；在下方连接指南的“要使用的模型名称”中输入后会自动生成命令。下载完成后点击“刷新状态”。",
      select_model:
        "在初始设置页面或 设置 > 对话模型 中选择设备端 AI 模式或本地 Ollama 模式，再选择一个模型。选择会被保存，并在所有页面以相同的值生效。",
      start_chat:
        "模型显示“运行中”即准备完毕。在大厅或对话页面选择精灵并打个招呼吧。第一次回复可能因加载模型而稍慢。",
    },
    guideStepStates: {
      checking: "确认中",
      done: "完成",
    ...
    chatModelSelectorDescription:
      "在设备端 AI 模式与本地 Ollama 模式中选择其一，再选择该模式下实际已连接的模型。初始设置与设置使用同一列表和同一保存值。",
    chatModelModeTitles: {
      on_device: "设备端 AI 模式",
      ollama: "本地 Ollama 模式",
    },
    ...
    chatModelModeEmpty: {
      on_device:
        "当前浏览器中没有可选择的设备端模型。请在 设置 > 对话模型 中准备 Chrome 设备端模型，或查看指南。",
      ollama:
        "Ollama 中没有可选择的模型。请按指南中的命令启动 Ollama 并下载模型，然后刷新。",
    },
    chatModelActiveMode: "当前对话模式",
    chatModelOptionCount: (count) => `可选择 ${count} 个`,
    chatModelOllamaLocalServerOnly:
      "只有通过 EVAI 本地服务器打开时才能使用本地 Ollama 模式。运行方法请查看指南。",
    chatModelSavedTo: (storageName) =>
      `所选模型保存在${storageName}中，并在所有页面以同一值生效。`,
    spiritActionNote: (spiritName, actions) =>
      `${spiritName}的行动：${actions}`,
    proactiveMessages: "精灵主动发消息",
    proactiveMessagesDescription:
      "开启后，正在聊天的精灵和其他精灵都会根据对话中累积的思念、好感、嫉妒以及各自的性格主动发来消息。主动的精灵可能在你回复前连续发送多条。",
    cheatMode: "作弊模式",
    cheatModeDescription:
      "开启后顶部会出现作弊模式页面，每位精灵的羁绊等级、性格、情绪与语气预设会应用到实际回复中。",
    cheatPageTitle: "精灵作弊设置",
    cheatPageDescription:
      "直接设定每位精灵的羁绊等级，并用预设切换基础性格、情绪与说话方式。更改从下一次回复起生效。",
    cheatSearchPlaceholder: "精灵名称或英文名",
    cheatNoSpirits: "没有符合条件的精灵。",
    cheatAppliedBadge: "生效中",
    cheatBondTitle: "羁绊等级",
    cheatBondDescription:
      "无论对话记录如何，关系阶段与羁绊显示都会固定为此等级。",
    ...
    setupModelSelectionHint:
      "现在选择对话模式与模型。这里选择的值与 设置 > 对话模型 使用同一保存值。",
    setupModelRequired: "需要选择一个模型才能开始。",
    setupModelLoading: "所选模型完全加载并运行后即可开始。",
    setupModelLoadFailed: (detail) =>
      `所选模型加载失败，无法开始。请重新选择模型或刷新列表。（${detail}）`,
    storageObjectKinds: {
      object_store: "对象存储",
      table: "数据表",
      view: "视图",
    ...
    rankingPageDescription:
      "依据累积对话与记忆计算的真实羁绊分数，比较所有精灵。",
    bondScoreLabel: "羁绊分数",
    memoryPageTitle: "记忆算法流程",
    memoryPageDescription:
      "从数据库中保存的全部对话分析关键词优先级、与其他精灵的关系和内心状态，构建下一次回复行为步骤的实际流程。",
    memoryGraphConnections: "连接线",
    memoryContextKinds: {
      conversation: "数据库对话流程",
      directive: "用户指示",
      episodic: "对话事件",
      habit: "关键词线索",
      affect: "情绪与吃醋",
    ...
    memoryGraphSelectHint:
      "滚轮缩放，拖动空白处平移，拖动节点调整布局。点击关键词或精灵，查看当时的对话与精灵的行动。",
    memoryNetworkViewAll: "全体关系图",
    memoryNetworkViewAllDescription:
      "在一个画面中查看所有有对话记录的精灵，并一次看清精灵之间的嫉妒。",
    memoryNetworkOnlySpirit: "按精灵查看",
    memoryNetworkEmpty:
      "还没有可绘制关系图的记录。与精灵对话后会填充全体关系图。",
    memoryNetworkNodeValue: (level, affection) =>
    ...
    memoryGraphEdgeSaviorBondLabel: (level, messageCount) =>
      `羁绊 Lv.${level} · 消息 ${messageCount}`,
    memoryGraphEdgeCanonBondLabel: (strength, sharedUnion) =>
      sharedUnion === null
        ? `原作羁绊 ${strength}`
        : `同属 ${sharedUnion} · 羁绊 ${strength}`,
    ...
    memoryGraphEdgeJealousyLabel: (
      ...
      [
        ...
        .join(" · "),
    memoryGraphPersonaHeartValue: (level, affection) =>
      affection === null ? `Lv.${level}` : `Lv.${level} · 爱意 ${affection}`,
    memoryGraphJealousyNodeValue: (strongestStir) =>
      `嫉妒最高 ${strongestStir}%`,
    memoryHeartTitle: "情绪起伏",
    memoryHeartDescription: (spiritName) =>
      `根据与${spiritName}保存的对话按日期累积计算的内心状态，直接用于主动搭话与嫉妒判断。`,
    memoryHeartMetrics: {
      affection: "爱意",
      trust: "信任",
      longing: "思念",
    ...
    memoryHeartExpressions: {
      openness: "心扉敞开",
      outward_warmth: "外露的温度",
      receptiveness: "接纳程度",
      initiative: "主动程度",
    },
    memoryHeartChartLabel: (spiritName) =>
      `${spiritName}按日期的爱意、信任、思念、伤心变化`,
    memoryHeartRivalBarLabel: "救援者发给其他精灵的消息",
    memoryHeartRivalCount: (count) => `其他精灵消息 ${count}`,
    memoryHeartContactSummary: (contactDays, saviorMessages) =>
      `对话天数 ${contactDays} · 救援者消息 ${saviorMessages}`,
    ...
    memoryRelationCanonStats: (interactionCount, mentionCount) =>
      `原作交流 ${interactionCount} 次 · 提及 ${mentionCount} 次`,
    memoryRelationSaviorBond: (level, messageCount) =>
      `与救援者的羁绊 Lv.${level} · 消息 ${messageCount} 条`,
    memoryRelationNoSaviorBond: "尚无与救援者的对话记录。",
    memoryKeywordDetailTitle: (token) => `关键词 · ${token}`,
    memoryKeywordStats: (userCount, spiritCount, firstSeen, lastSeen) =>
      `救援者 ${userCount} 次 · 精灵 ${spiritCount} 次 · 首次 ${firstSeen} · 最近 ${lastSeen}`,
    ...
    memoryRivalStats: (userCount, spiritCount, firstAt, latestAt) =>
      `救援者发送 ${userCount} 条 · 回复 ${spiritCount} 条 · ${firstAt} ~ ${latestAt}`,
    ...
    memoryBehaviorStages: {
      ...
      keywords: {
        title: "关键词线索",
        description: "与输入和最近脉络相关的优先关键词",
      },
      recall: {
        title: "记忆召回",
        description: "整合记忆与相关事件（按时间顺序）",
      },
      social: {
        title: "其他精灵分析",
        description: "引发吃醋与竞争的对话记录",
      },
      inner_state: {
        title: "内心状态",
        description: "精灵自己整理的心情与下一步意图",
      },
    ...
    modulesSectionDescription:
      "已启用模块的说明与设定集内容会添加到聊天系统提示词中。",
    moduleDelete: "删除模块",
    moduleStats: (lorebook, regex, trigger) =>
      `设定集 ${lorebook} · 正则 ${regex} · 触发器 ${trigger}`,
...
function createLabelBag(
  labelsByLanguage: Record<AppLanguage, EverTalkLabels>,
): EverTalkLabelBag {
  const labelKeys = Object.keys(labelsByLanguage.ko) as Array<
    keyof EverTalkLabels
  >;
  const bag: Partial<
    Record<keyof EverTalkLabels, Record<AppLanguage, unknown>>
  > = {};
...
export const EVERTALK_LABEL_BAG: EverTalkLabelBag =
  createLabelBag(EVERTALK_LABELS);
