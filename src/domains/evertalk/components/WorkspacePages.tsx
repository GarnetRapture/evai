...
function spiritName(controller: EverTalkController, personaId: string): string {
  const spirit = controller.allSpirits.find(
    (candidate) => candidate.id === personaId,
  );
  return spirit
    ? parseSpiritDetail(spirit, controller.appLanguage).name
    : personaId;
}
...
export function StorageAnalyticsPage({ controller }: WorkspacePageProps) {
  ...
  const totalStoreBytes =
    inspection?.stores.reduce((sum, store) => sum + store.estimated_bytes, 0) ??
    0;
  const maxPersonaBytes = Math.max(
    1,
    ...personaRows.map((persona) => persona.estimated_bytes),
  );
  const messageStore = inspection?.stores.find(
    (store) => store.store_name === "chat_message",
  );
  const memoryStore = inspection?.stores.find(
    (store) => store.store_name === "persona_memory",
  );
  const compositionTotal = Math.max(
    1,
    (messageStore?.estimated_bytes ?? 0) + (memoryStore?.estimated_bytes ?? 0),
  );
  const messageAngle =
    ((messageStore?.estimated_bytes ?? 0) / compositionTotal) * 360;
  return (
    <WorkspaceSurface controller={controller} labelledBy="storage-page-title">
      <header className="ever-workspace-page__header">
        <div>
          <p>{labels.navStorage}</p>
          <h1 id="storage-page-title">{labels.storagePageTitle}</h1>
          <span>{labels.storagePageDescription}</span>
        </div>
        <button
          type="button"
          disabled={controller.storageInspectionLoading}
          onClick={() => void controller.refreshStorageInspection()}
        >
          <RefreshCw
            size={17}
            className={controller.storageInspectionLoading ? "is-spinning" : ""}
          />
          {labels.refreshAnalysis}
        </button>
      </header>
      {controller.storageInspectionError ? (
        <div className="ever-workspace-error">
          {controller.storageInspectionError}
        </div>
      ) : null}
      {controller.storageWriteMessage ? (
        <div className="ever-workspace-notice">
          {controller.storageWriteMessage}
        </div>
      ) : null}
      <section className="ever-storage-location-grid">
        <article className="ever-insight-card is-active">
          <div className="ever-insight-card__title">
            <Database size={20} />
            <strong>{labels.browserManagedLocation}</strong>
          </div>
          <p>{labels.browserManagedLocationDetail}</p>
          <dl>
            <div>
              <dt>Origin</dt>
              <dd>{inspection?.origin || "-"}</dd>
            </div>
            <div>
              <dt>
                {
                  labels.storageBackendName[
                    inspection?.backend ?? controller.storageKind
                  ]
                }
              </dt>
              <dd>{inspection?.database_name || "-"}</dd>
            </div>
            {inspection?.engine_version ? (
              <div>
                <dt>SQLite</dt>
                <dd>{inspection.engine_version}</dd>
              </div>
            ) : null}
            {inspection?.server_version ? (
              <div>
                ...
                <dd>
                  {inspection.server_version}
                  {inspection.server_port === null
                    ? ""
                    : ` · :${inspection.server_port}`}
                </dd>
              </div>
            ) : null}
            {inspection?.link_row_count === null ||
            inspection?.link_row_count === undefined ? null : (
              <div>
      ...
      <section className="ever-metric-grid">
        <article>
          <small>{labels.storageModeActive}</small>
          <strong>
            {
              labels.storageBackendName[
                inspection?.backend ?? controller.storageKind
        ...
        <article>
          <small>{labels.storageUsage}</small>
          <strong>
            {formatBytes(inspection?.usage_bytes ?? null, locale)}
          </strong>
        </article>
        <article>
          <small>{labels.storageQuota}</small>
          <strong>
            {formatBytes(inspection?.quota_bytes ?? null, locale)}
          </strong>
        </article>
        <article>
          <small>{labels.snapshotEstimate}</small>
          <strong>
            {formatBytes(inspection?.estimated_snapshot_bytes ?? null, locale)}
          </strong>
        </article>
      </section>
      <section className="ever-analytics-grid">
        <article className="ever-chart-card">
          ...
          <div className="ever-donut-wrap">
            <div
              className="ever-donut"
              style={{
                background: `conic-gradient(#7e67d9 0deg ${messageAngle}deg, #e68ea7 ${messageAngle}deg 360deg)`,
              }}
            >
              <span>
                {formatBytes(
                  (messageStore?.estimated_bytes ?? 0) +
                    (memoryStore?.estimated_bytes ?? 0),
                  locale,
                )}
              </span>
            </div>
            <div className="ever-chart-legend">
              <span>
                <i className="is-message" />
                {labels.messagesLabel} {messageStore?.record_count ?? 0}
              </span>
              <span>
                <i className="is-memory" />
                {labels.memoriesLabel} {memoryStore?.record_count ?? 0}
              </span>
            </div>
          </div>
        </article>
        <article className="ever-chart-card">
          ...
          <div className="ever-bar-list">
            {inspection?.stores.map((store) => (
              <div key={store.physical_name}>
                <span>
                  <b>{store.store_name}</b>
                  <small>
                    {store.record_count} {labels.recordsLabel} ·{" "}
                    {formatBytes(store.estimated_bytes, locale)}
                  </small>
                </span>
                <i>
                  <em
                    style={{
                      width: `${totalStoreBytes ? Math.max(2, (store.estimated_bytes / totalStoreBytes) * 100) : 0}%`,
                      backgroundImage: `url(${DECOR_UI_ASSETS.gaugeGradient})`,
                    }}
                  />
                </i>
              </div>
      ...
      <section className="ever-storage-schema">
        ...
        {!inspection?.stores.length ? (
          <p>{labels.noStoredData}</p>
        ) : (
          <div className="ever-storage-schema__grid">
            {inspection.stores.map((store) => (
              <StorageStoreCard
        ...
        {!personaRows.length ? (
          ...
          personaRows.map((persona) => (
            <details
              ...
              className="ever-persona-storage__row"
            >
              <summary>
                <SpiritViewAvatar
                  controller={controller}
                ...
                />
                <span>
                  <strong>{spiritName(controller, persona.persona_id)}</strong>
                  <small>
                    {labels.messagesLabel} {persona.message_count} ·{" "}
                    {labels.memoriesLabel} {persona.memory_count}
                  </small>
                </span>
                <i>
                  <em
                    style={{
                      width: `${Math.max(2, (persona.estimated_bytes / maxPersonaBytes) * 100)}%`,
                    }}
                  />
                </i>
                <b>{formatBytes(persona.estimated_bytes, locale)}</b>
              </summary>
              <div className="ever-persona-storage__samples">
                <h3>{labels.storedContents}</h3>
                {persona.samples.map((sample) => (
                  <article key={`${sample.kind}-${sample.id}`}>
                    <span>
                      {sample.kind === "message"
                    ...
                    <p>{sample.content}</p>
                    <time>
                      {new Date(sample.created_at).toLocaleString(locale)}
                    </time>
                  </article>
                ))}
              </div>
...
export function BondRankingPage({ controller }: WorkspacePageProps) {
  const scores = new Map(
    controller.bondRanking.map((entry) => [entry.persona_id, entry]),
  );
  ...
  return (
    <WorkspaceSurface controller={controller} labelledBy="ranking-page-title">
      <header className="ever-workspace-page__header">
        <div>
          <p>{controller.labels.navRanking}</p>
          <h1 id="ranking-page-title">{controller.labels.rankingPageTitle}</h1>
          <span>{controller.labels.rankingPageDescription}</span>
        </div>
        <Trophy size={34} />
      </header>
      <section className="ever-ranking-hero">
        {ranking.slice(0, 3).map((entry, index) => (
          <article key={entry.personaId} className={`rank-${index + 1}`}>
            <img
              className="ever-ranking-hero__frame"
              src={
                index === 0
                  ? LOBBY_UI_ASSETS.emptySlotCrowned
                  : LOBBY_UI_ASSETS.gradeBloom
              }
              alt=""
              aria-hidden="true"
            />
            ...
            <small>{entry.grade}</small>
            <span>
              {controller.labels.bondScoreLabel} {entry.score}
            </span>
          </article>
        ))}
      </section>
      <section className="ever-ranking-table">
        {ranking.map((entry, index) => (
          <article key={entry.personaId}>
            <b>{index + 1}</b>
            <SpiritViewAvatar
              controller={controller}
              personaId={entry.personaId}
            ...
            <div>
              <strong>{entry.name}</strong>
              <small>
                {entry.grade} · {controller.labels.messagesLabel}{" "}
                {entry.messages} · {controller.labels.memoriesLabel}{" "}
                {entry.memories}
              </small>
              <i>
                <em
                  style={{
                    width: `${(entry.score / maximum) * 100}%`,
                    backgroundImage: `url(${LOBBY_UI_ASSETS.gaugeFill})`,
                  }}
                />
              </i>
            </div>
...
export function MemoryWorkflowPage({ controller }: WorkspacePageProps) {
  ...
  const threads = useMemo(
    () =>
      contextGraph === null
        ? []
        : filterMemoryKeywordThreads(contextGraph.keyword_threads, {
            query,
            recentOnly,
          }),
  ...
  const graphSpiritName =
    contextGraph === null
      ? ""
      : spiritName(controller, contextGraph.persona_id);
    ...
    () =>
      new Map(
        allSpirits.map((spirit) => [
          spirit.id,
          parseSpiritDetail(spirit, appLanguage).name,
        ]),
      ),
      ...
      contextGraph === null
        ...
        : buildMemoryContextGraphLayout(
            contextGraph,
            {
              ...
              resolve_spirit_name: (personaId) =>
                spiritNames.get(personaId) ?? personaId,
            },
            threads,
            labels,
          ),
  ...
  const selectedThread =
    selection?.kind === "keyword"
      ? (threads.find(
          (thread) => `keyword:${thread.keyword.token}` === selection.id,
        ) ?? null)
      : null;
  const selectedRelation =
    selection?.kind === "relation"
      ? (contextGraph?.relations.find(
          (relation) =>
            `relation:${relation.relation.character_key}` === selection.id,
        ) ?? null)
      : null;
  ...
  return (
    <WorkspaceSurface
      controller={controller}
      labelledBy="memory-page-title"
      layout="canvas"
    >
      <header className="ever-workspace-page__header">
        <div>
          <p>{labels.navMemory}</p>
          <h1 id="memory-page-title">{labels.memoryPageTitle}</h1>
          <span>{labels.memoryPageDescription}</span>
        </div>
        <button
          type="button"
          disabled={
            controller.contextGraphLoading ||
            controller.contextGraphPersonaId.length === 0
          }
          onClick={() => void controller.refreshContextGraph()}
        >
          <RefreshCw
            size={17}
            className={controller.contextGraphLoading ? "is-spinning" : ""}
          />
          {labels.refreshAnalysis}
        </button>
      </header>
      <section
        className="ever-memory-filter"
        aria-labelledby="memory-network-title"
      >
        <div className="ever-memory-filter__head">
          <strong id="memory-network-title">
            {labels.memoryNetworkViewAll}
          </strong>
          <span>{labels.memoryNetworkViewAllDescription}</span>
        </div>
        <div className="ever-memory-filter__view">
          <button
            type="button"
            className={networkActive ? "is-on" : ""}
            aria-pressed={networkActive}
            onClick={() => void controller.viewRelationshipNetwork()}
          >
            {labels.memoryNetworkViewAll}
          </button>
          <button
            ...
            aria-pressed={!networkActive}
            onClick={() =>
              void controller.viewContextGraphPersona(
                controller.contextGraphPersonaId,
              )
            }
          >
            {labels.memoryNetworkOnlySpirit}
          </button>
        </div>
      </section>
      {networkActive ? (
        networkGraph === null ? (
          <p className="ever-memory-graph-empty">
            {controller.relationshipNetworkLoading
              ? labels.checking
              : labels.memoryNetworkEmpty}
          </p>
        ) : (
        ...
        <>
          ...
          >
            <div className="ever-memory-filter__head">
              <strong id="memory-filter-title">
                {labels.memoryFilterTitle}
              </strong>
            ...
            <div className="ever-memory-filter__kinds">
              {MEMORY_CONTEXT_KINDS.map((kind) => (
                <button
                  ...
                  onClick={() =>
                    void controller.setMemoryContextEnabled(
                      kind,
                      !memoryContextFilter[kind],
                    )
                  }
                >
            ...
            <div className="ever-memory-filter__view">
              <label className="ever-memory-filter__search">
                ...
                <input
                  ...
                  aria-label={labels.memoryFilterSearchPlaceholder}
          ...
          {contextGraph === null || graph === null ? (
            ...
            <div className="ever-memory-workbench">
              <MemoryGraphCanvas
                ...
                hint={labels.memoryGraphSelectHint}
                emptyMessage={
                  threads.length === 0 ? labels.memoryFilterEmpty : null
                }
