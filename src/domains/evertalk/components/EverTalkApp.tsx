...
import { buildMoodAccent } from "../logic";
...
export function EverTalkApp() {
  ...
  if (controller.platformSupport !== "supported") {
    return (
      <PlatformBlockedPanel
        reason={controller.platformSupport}
        ...
        {controller.workspaceView === "guide" ? (
          ...
          <SetupWizard
            ...
            onPlatformGuideConfirmedChange={
              controller.setPlatformGuideConfirmed
            }
  ...
  const familiaritySpirit = controller.activeFamiliarityEntry
    ? controller.filteredSpirits.find(
        (spirit) => spirit.id === controller.activeFamiliarityEntry?.persona_id,
      )
    : undefined;
  const familiarityDetail = familiaritySpirit
    ? parseSpiritDetail(familiaritySpirit, controller.appLanguage)
    : null;
  async function openFamiliarityChat(personaId: string) {
    const spirit = controller.filteredSpirits.find(
  ...
  return (
    <>
      ...
      {controller.workspaceView === "chat" ? (
        <div
          className={`ever-app-shell ${controller.profileCollapsed ? "is-profile-collapsed" : ""} ${controller.rosterCollapsed ? "is-roster-collapsed" : ""}`}
        >
          <SpiritRoster
            ...
            onToggleCollapsed={() =>
              controller.setRosterCollapsed(!controller.rosterCollapsed)
            }
          ...
          <ChatStage
            ...
            messagesListRef={controller.messagesListRef}
            labels={controller.labels}
            ...
            moodAccent={buildMoodAccent(
              controller.memoryOverview?.entries.find(
                (entry) => entry.persona_id === controller.activeSpiritId,
              )?.emotion ?? null,
            )}
          />
          <SpiritProfilePanel
            ...
            labels={controller.labels}
            onToggleCollapsed={() =>
              controller.setProfileCollapsed(!controller.profileCollapsed)
            }
      ...
      {controller.workspaceView === "chat" && (
        <button
          type="button"
          className="ever-lobby-fab"
          aria-label={controller.labels.lobby}
          aria-pressed={controller.lobbyOpen}
          onClick={
            controller.lobbyOpen ? controller.closeLobby : controller.openLobby
          }
        >
          <img
            className="ever-lobby-fab__icon"
            src={EVERTALK_UI_ASSETS.lobbyPointer}
            alt=""
            aria-hidden="true"
          />
