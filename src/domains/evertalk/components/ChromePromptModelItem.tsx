...
import type { ChromePromptModelItemProps } from '../types';

export function ChromePromptModelItem({ entry, modelPreparation, modelLoadingId, labels, onSelectChatModel, onPrepareChromePromptModel }: ChromePromptModelItemProps) {
    ...
    return (<div className={`ever-model-item ${entry.selected ? 'is-selected' : ''}`}>
        ...
        {needsPreparation ? (<button type="button" className="ever-settings-reset-button" disabled={modelPreparation !== null && modelPreparation.error === null} onClick={() => void onPrepareChromePromptModel(entry)}>
