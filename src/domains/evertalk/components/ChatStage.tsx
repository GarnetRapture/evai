...
const GalleryTile = memo(function GalleryTile({
  ...
  return (
    <button
      ...
      aria-label={`${skinLabel} ${zoomLabel}`}
      onClick={() => onZoom(skin.portraitCandidates)}
    >
      <LoadableAssetImage
        candidates={skin.portraitCandidates}
        alt={spiritName}
        fallback={<span>{skinLabel}</span>}
      />
...
const ChatMessageBubble = memo(function ChatMessageBubble({
  ...
  if (message.role === "system") {
    return (
      <div className="ever-message is-system">
  ...
  return (
    <div className={`ever-message ${fromUser ? "is-user" : "is-spirit"}`}>
      {!fromUser && (
        <div className="ever-message__avatar">
          <LoadableAssetImage
            candidates={avatarCandidates}
            alt={spiritName}
            fallback={<span>{spiritName.charAt(0) || "E"}</span>}
          />
        </div>
      )}
      ...
      <button
        type="button"
        className="ever-message__delete"
...
export function ChatStage({
  ...
  const tone = useMemo(
    () => (activeDetail ? getRaceTone(activeDetail.race) : "tone-neutral"),
    [activeDetail],
  );
  ...
  const canSubmitMessage =
    canUseComposer && !isTyping && inputText.trim().length > 0;
  const [portraitReaction, setPortraitReaction] = useState({
    sourceLine: speechLine,
    displayLine: speechLine,
    poked: false,
  });
  const currentPortraitReaction =
    portraitReaction.sourceLine === speechLine
      ? portraitReaction
      : { sourceLine: speechLine, displayLine: speechLine, poked: false };
  const [zoomedImageCandidates, setZoomedImageCandidates] = useState<
    string[] | null
  >(null);
  ...
  useEffect(() => {
    ...
    const redirectTypingToComposer = (event: KeyboardEvent) => {
      ...
      if (
        ...
        event.metaKey ||
        event.altKey ||
        isEditableInteractionTarget(event.target)
      ) {
        return;
      }
    ...
    return () =>
      document.removeEventListener("keydown", redirectTypingToComposer, true);
  ...
  const panelShakeTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  function movePanelDrag(event: React.PointerEvent<HTMLElement>) {
    ...
    const blockedX =
      maxX > 0 && Math.abs(rawX - clampedX) > PANEL_BOUNDARY_TOLERANCE_PX;
    const blockedY =
      maxY > 0 && Math.abs(rawY - clampedY) > PANEL_BOUNDARY_TOLERANCE_PX;
    ...
    const observer = new ResizeObserver(() => {
      setPanelSize((currentSize) => {
        ...
        const width = Math.min(
          currentSize.width,
          Math.max(CHAT_PANEL_MIN_WIDTH, parent.clientWidth),
        );
        const height = Math.min(
          currentSize.height,
          Math.max(CHAT_PANEL_MIN_HEIGHT, parent.clientHeight),
        );
      ...
      setPanelPos((currentPos) => {
        ...
        const maxX = Math.max(
          0,
          parent.clientWidth - (panel?.offsetWidth ?? 0),
        );
        const maxY = Math.max(
          0,
          parent.clientHeight - (panel?.offsetHeight ?? 0),
        );
  ...
  function handlePortraitPoke() {
    ...
    setPortraitReaction({
      sourceLine: speechLine,
      displayLine: pickPokeReactionLine(
        activeDetail,
        currentPortraitReaction.displayLine,
      ),
      poked: true,
    });
    if (pokeTimeoutRef.current) {
    ...
    pokeTimeoutRef.current = setTimeout(() => {
      setPortraitReaction((current) =>
        current.sourceLine === speechLine
          ? { ...current, poked: false }
          : current,
      );
    }, 1600);
  ...
  return (
    <main className={`ever-stage ${tone}`}>
      {assets && (
        <img
          className="ever-stage__background"
      ...
      <header className="ever-stage__header">
        <div>
          ...
          <button
            className="ever-stage__profile-button"
            type="button"
        ...
        <div
          className={`ever-engine ${llmStatus?.is_loaded ? "is-on" : "is-off"}`}
        >
          <span />
      ...
      <section className="ever-stage__body">
        <div className="ever-character" key={activeDetail?.id ?? "none"}>
          <div
            className={`ever-character__figure ${currentPortraitReaction.poked ? "is-poked" : ""}`}
            style={
              moodAccent
                ? ({
                    "--ever-mood-color": moodAccent.color,
                    "--ever-mood-glow": moodAccent.glow,
                  } as React.CSSProperties)
                : undefined
            }
          >
            <span className="ever-character__aura" aria-hidden="true" />
            {currentPortraitReaction.displayLine && (
              <div className="ever-character__speech">
                {currentPortraitReaction.displayLine}
              </div>
            )}
            <button
              ...
              aria-label={
                activeDetail
                  ? `${activeDetail.name} ${labels.spiritReaction}`
                  : labels.spiritReaction
              }
              disabled={!activeDetail}
              onClick={handlePortraitPoke}
            >
              <LoadableAssetImage
                candidates={activeSkin?.portraitCandidates ?? []}
                alt={activeDetail?.name ?? ""}
                className="ever-character__portrait"
                fallback={
                  <div className="ever-character__fallback">
                    {activeDetail?.name.charAt(0) ?? "E"}
                  </div>
                }
              />
          ...
          <div className="ever-character__overlay">
            {activeDetail && (
              <button
                type="button"
                className="ever-character__caption ever-character__caption--button"
                onClick={onOpenProfileDetail}
              >
                <strong>{activeDetail.name_en}</strong>
                <span>
                  {activeDetail.profile.nick_name ?? activeDetail.race}
                </span>
              </button>
            )}
          </div>
        ...
        {activeStageTab === "chat" ? (
          <div
            ref={panelRef}
            className={`ever-chat-panel is-${panelState} ${panelPos && panelState === "normal" ? "is-floating" : ""} ${panelShake ? "is-shake" : ""}`}
            style={
              panelState === "normal"
                ? {
                    ...(panelSize
          ...
          >
            <div
              className="ever-chat-panel__room ever-chat-panel__room--draggable"
            ...
            >
              <strong>
                {activeRoom
                  ? formatRoomTitle(activeRoom, labels)
                  : (activeDetail?.name ?? labels.bondChannel)}
              </strong>
              <span>{summary}</span>
              <div
                className="ever-chat-panel__window-controls"
                onPointerDown={(event) => event.stopPropagation()}
              >
                <button
                  type="button"
                  aria-label={labels.windowMinimize}
                ...
                <button
                  ...
                  aria-label={
                    panelState === "maximized"
                      ? labels.windowRestore
                      : labels.windowMaximize
                  }
              ...
              <div
                className="ever-chat-panel__room-actions"
                onPointerDown={(event) => event.stopPropagation()}
              >
                <button
                  type="button"
                ...
                <button
                  type="button"
                  disabled={!activeDetail}
                  aria-label={labels.previousChats}
              ...
              {historyOpen && (
                <div
                  className="ever-chat-panel__history"
                  onPointerDown={(event) => event.stopPropagation()}
                >
                  {previousRoomsLoading && (
                    <span className="ever-chat-panel__history-loading">…</span>
                  )}
                  {!previousRoomsLoading && previousRooms.length === 0 && (
                    <span className="ever-chat-panel__history-empty">
                  ...
                  {!previousRoomsLoading &&
                    previousRooms.map((room) => (
                      <div
                        key={room.id}
                        className={`ever-chat-panel__history-item ${room.id === activeRoom?.id ? "is-active" : ""}`}
                      >
                        <button
                          type="button"
                          onClick={async () => {
                        ...
                        <button
                          ...
                          onClick={(event) => handleDeleteRoom(event, room.id)}
                        >
            ...
            <div className="ever-messages" ref={messagesListRef}>
              {openingGreeting.length > 0 && (
                <div className="ever-message is-spirit is-opening">
                  <div className="ever-message__avatar">
                    <LoadableAssetImage
                      candidates={
                        activeSkin?.avatarCandidates ??
                        assets?.avatarCandidates ??
                        []
                      }
                      alt={activeDetail?.name ?? ""}
                      fallback={
                        <span>{activeDetail?.name.charAt(0) ?? "E"}</span>
                      }
                    />
                  </div>
                  <div className="ever-message__bubble">
                    <span className="ever-message__text">
                      {openingGreeting}
                    </span>
                  </div>
                </div>
              ...
              {messages.length === 0 && openingGreeting.length === 0 && (
                <div className="ever-messages__empty">
              ...
              {messages.map((message, index) => (
                <ChatMessageBubble
                  ...
                  showActionStatus={shouldAnnounceSpiritActions(
                    message,
                    index,
                    messages.length,
                  )}
            ...
            <form className="ever-composer" onSubmit={onSendMessage}>
              ...
              <input
                ...
                enterKeyHint="send"
                placeholder={
                  activeDetail && llmStatus?.is_loaded
                    ? labels.messagePlaceholder(activeDetail.name)
                    : labels.modelRequiredPlaceholder
                }
              />
              {streamingRequestId ? (
                <button
            ...
            {panelState === "normal" &&
              CHAT_PANEL_RESIZE_HANDLES.map((handle) => (
                <div
                  key={handle}
          ...
          <div className="ever-gallery-panel">
      ...
      <nav
        className="ever-tabbar ever-stage-tabs"
        aria-label={labels.rosterTitle}
      >
        <button
          className={activeStageTab === "chat" ? "is-active" : ""}
          type="button"
          onClick={() => onStageTabChange("chat")}
        >
          <span className="ever-tabbar__icon">
            <img src={EVERTALK_UI_ASSETS.tabChat} alt="" aria-hidden="true" />
            <img
              className="is-pressed"
              src={EVERTALK_UI_ASSETS.tabChatPressed}
              alt=""
              aria-hidden="true"
            />
          </span>
        ...
        <button
          className={activeStageTab === "gallery" ? "is-active" : ""}
          type="button"
          onClick={() => onStageTabChange("gallery")}
        >
          <span className="ever-tabbar__icon">
            <img
              src={EVERTALK_UI_ASSETS.tabGallery}
              alt=""
              aria-hidden="true"
            />
            <img
              className="is-pressed"
              src={EVERTALK_UI_ASSETS.tabGalleryPressed}
              alt=""
              aria-hidden="true"
            />
          </span>
