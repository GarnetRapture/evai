...
import {
  ...
  inspectDeviceEnvironment,
  type DeviceEnvironmentInfo,
...
import {
  ...
  PROACTIVE_INITIAL_DELAY_MS,
  chatClient,
  type ChatMessage,
  type ChatRoom,
  type MemoryContextKind,
  type PersonaContextGraph,
  type PersonaMemoryInsight,
...
import {
  DEFAULT_SPIRIT_SKIN_ID,
  getSpiritVisualAssets,
  parseSpiritDetail,
  personaClient,
  type BondRankingEntry,
...
import {
  applyStorageRecordWrite,
  inspectBrowserStorage,
  readStorageRecords,
  syncClient,
  type BackupDirectoryStatus,
  type BrowserStorageInspection,
...
export function useFirstLoadableImage(
  ...
  const [selection, setSelection] = useState({ candidatesKey, index: 0 });
  const index = selection.candidatesKey === candidatesKey ? selection.index : 0;
  ...
  return [
    ...
    () =>
      setSelection((current) => ({
        candidatesKey,
        index: current.candidatesKey === candidatesKey ? current.index + 1 : 1,
      })),
  ];
}
export function useEverTalkController(): EverTalkController {
  ...
  const [platformSupport] = useState<PlatformSupportStatus>(
    detectPlatformSupport,
  );
  ...
  const [proactiveUnreadCounts, setProactiveUnreadCounts] = useState<
  ...
  const [activeFamiliarityEntry, setActiveFamiliarityEntry] =
    useState<FamiliarityEntry | null>(null);
  const [memoryInsight, setMemoryInsight] =
    useState<PersonaMemoryInsight | null>(null);
  const [memoryInsightLoading, setMemoryInsightLoading] = useState(false);
  const [memoryOverview, setMemoryOverview] =
    useState<PersonaMemoryOverview | null>(null);
  const [memoryOverviewLoading, setMemoryOverviewLoading] = useState(false);
  const [contextGraph, setContextGraph] = useState<PersonaContextGraph | null>(
    null,
  );
  ...
  const [relationshipNetwork, setRelationshipNetwork] =
    useState<PersonaRelationshipNetwork | null>(null);
  const [relationshipNetworkLoading, setRelationshipNetworkLoading] =
    useState(false);
  const [relationshipNetworkActive, setRelationshipNetworkActive] =
    useState(false);
  const [lobbyOpen, setLobbyOpen] = useState(true);
  const [lobbyBackgroundPickerOpen, setLobbyBackgroundPickerOpen] =
    useState(false);
  ...
  const gatePending =
    (appSettings?.setup_stage ?? "language") !== "done" ||
    !(appSettings?.platform_guide_acknowledged ?? false);
  const [userSession, setUserSession] = useState<UserSession | null>(null);
  const [deviceEnvironment, setDeviceEnvironment] =
    useState<DeviceEnvironmentInfo | null>(null);
  const [modelCatalog, setModelCatalog] = useState<ChatModelCatalog | null>(
    null,
  );
  ...
  const [modelPreparation, setModelPreparation] =
    useState<ModelPreparationState | null>(null);
  const [chromeInstalledModelLinking, setChromeInstalledModelLinking] =
    useState(false);
  const [hostRuntime] = useState(readAppHostRuntime);
  const [storageKind] = useState(readAppStorageKind);
  const [ollamaGuideVisible] = useState(
    () => appPlatform === "web_chrome" && isLocalServerRuntime(),
  );
  const [localServerNoticeVisible] = useState(
    () => appPlatform === "web_chrome" && !isLocalServerRuntime(),
  );
  const [backupBusy, setBackupBusy] = useState(false);
  const [backupMessage, setBackupMessage] = useState<string | null>(null);
  const [backupError, setBackupError] = useState<string | null>(null);
  const [backupDirectoryStatus, setBackupDirectoryStatus] =
    useState<BackupDirectoryStatus | null>(null);
  const [llmSessionStatuses, setLlmSessionStatuses] = useState<
    LlmSessionStatus[]
  >([]);
  const [llmRequestStatuses, setLlmRequestStatuses] = useState<
    LlmRequestStatus[]
  >([]);
  ...
  const [localStatus, setLocalStatus] = useState<LocalStatusSnapshot | null>(
    null,
  );
  const [workspaceView, setWorkspaceView] = useState<WorkspaceView>("chat");
  const [storageInspection, setStorageInspection] =
    useState<BrowserStorageInspection | null>(null);
  const [storageInspectionLoading, setStorageInspectionLoading] =
    useState(false);
  const [storageInspectionError, setStorageInspectionError] = useState<
    string | null
  >(null);
  const [storageRecords, setStorageRecords] = useState<
    Record<string, StorageRecordPage>
  >({});
  const [storageRecordsLoading, setStorageRecordsLoading] = useState<
    string | null
  >(null);
  const [storageWriteBusy, setStorageWriteBusy] = useState(false);
  const [storageWriteMessage, setStorageWriteMessage] = useState<string | null>(
    null,
  );
  ...
  const generationEngineLimits = useMemo(
    () => buildGenerationEngineLimits(modelCatalog),
    [modelCatalog],
  );
  ...
  function relocalizeSystemStatuses(uiLabels: EverTalkLabels) {
    setSystemStatuses((current) =>
      current.map((status) => {
        switch (status.id) {
          case "persona-archive":
            return createApiStatus(
              "persona-archive",
              spirits.length > 0 ? "ready" : status.state,
              spirits.length > 0
                ? uiLabels.archiveCount(spirits.length)
                : uiLabels.checking,
            );
          case "persona-db": {
            ...
            return createApiStatus(
              "persona-db",
              count > 0 ? "ready" : "warning",
              count > 0 ? uiLabels.loadedCount(count) : uiLabels.noDbRows,
            );
          }
          case "chat-db":
            return localStatus
              ? createApiStatus(
                  "chat-db",
                  "ready",
                  uiLabels.roomMessageCount(
                    localStatus.chat_room_count,
                    localStatus.chat_message_count,
                  ),
                )
          ...
          case "llm":
            return createApiStatus(
              "llm",
              llmStatus?.is_loaded ? "ready" : "warning",
              describeChatModelStatus(
                llmStatus,
                appSettings?.active_model ?? "",
                uiLabels,
              ),
            );
          case "context-storage":
            return createApiStatus(
              "context-storage",
              "ready",
              uiLabels.storageBackendName[readAppStorageKind()],
            );
          default:
            return status;
        }
  ...
  async function refreshEnvironment() {
    ...
    setSystemStatus(
      createApiStatus(
        "context-storage",
        "ready",
        labels.storageBackendName[storageKind],
      ),
    );
  }
  async function refreshLlmStatus() {
    ...
    try {
      ...
      setSystemStatus(
        createApiStatus(
          "llm",
          status.is_loaded ? "ready" : "warning",
          describeChatModelStatus(
            status,
            (await settingsClient.get()).active_model,
            labels,
          ),
        ),
      );
  ...
  async function readLlmStatus() {
    try {
      ...
      setLlmStatus({
        ...EMPTY_LLM_STATUS,
        error_message: formatUnknownError(err, labels),
      });
    }
  }
  async function ensureLlmReadyForPersonaCache(): Promise<boolean> {
    try {
      ...
      setSystemStatus(
        createApiStatus("llm", "error", formatUnknownError(err, labels)),
      );
      return false;
    }
  }
    ...
    try {
      ...
      setSystemStatus(
        createApiStatus(
          "chat-db",
          "ready",
          labels.roomMessageCount(
            snapshot.chat_room_count,
            snapshot.chat_message_count,
          ),
        ),
      );
      setSystemStatus(
        createApiStatus(
          "persona-db",
          snapshot.persona_count > 0 ? "ready" : "warning",
          labels.loadedCount(snapshot.persona_count),
        ),
      );
    } catch (err) {
      setSystemStatus(
        createApiStatus("chat-db", "error", formatUnknownError(err, labels)),
      );
    }
        ...
        .then((list) => {
          setSystemStatus(
            createApiStatus(
              "persona-archive",
              "ready",
              initLabels.archiveCount(list.length),
            ),
          );
        })
        .catch((err) => {
          setSystemStatus(
            createApiStatus(
              "persona-archive",
              "error",
              formatUnknownError(err, labels),
            ),
          );
        }),

      Promise.all([personaClient.list(), personaClient.getDefault()])
        .then(([list, defId]) => {
          dbList = list;
          ...
          setSystemStatus(
            createApiStatus(
              "persona-db",
              list.length > 0 ? "ready" : "warning",
              list.length > 0
                ? initLabels.loadedCount(list.length)
                : initLabels.noDbRows,
            ),
          );
        })
        ...
        .then((rooms) => {
          setSystemStatus(
            createApiStatus(
              "chat-db",
              "ready",
              initLabels.roomCount(rooms.length),
            ),
          );
        })
        .catch((err) => {
        ...
        .then((status) => {
          ...
          setSystemStatus(
            createApiStatus(
              ...
              status.is_loaded ? "ready" : "warning",
              status.is_loaded
                ? initLabels.modelLoaded
                : initLabels.modelAvailabilityDetail(status.availability),
            ),
          );
        })
        .catch((err) => {
  ...
  async function completeSetup() {
    ...
    try {
      ...
      const setupLanguage = appSettings?.language_configured
        ? appSettings.language
        : appLanguage;
      const staged = await settingsClient.completeInitialSetup(
        setupLanguage,
        setSetupProgress,
      );
  ...
  async function focusSpiritModelSession(spiritId: string) {
    ...
    try {
      ...
      setSystemStatus(
        createApiStatus("llm", "warning", formatUnknownError(err, labels)),
      );
    }
  ...
  async function loadRelationshipNetwork() {
    setRelationshipNetworkLoading(true);
    try {
      setRelationshipNetwork(await chatClient.getPersonaRelationshipNetwork());
    } catch (err) {
      console.error(labels.logMemoryInsightFailed, err);
    } finally {
      setRelationshipNetworkLoading(false);
    }
  }
  async function viewRelationshipNetwork() {
    setRelationshipNetworkActive(true);
    await loadRelationshipNetwork();
  }
  async function viewRelationshipNetworkPersona(personaId: string) {
    setRelationshipNetworkActive(false);
  ...
  async function selectSpirit(
    spirit: PersonaConfig,
    languageOverride?: AppLanguage,
    markProactiveRead = true,
  ) {
  ...
  async function toggleDefaultSpirit(spiritId: string) {
    try {
      const nextSettings =
        await settingsClient.togglePreferredPersona(spiritId);
      setDefaultPersonaId(nextSettings.default_persona_id);
      setAppSettings(nextSettings);
      const spiritName =
        spirits.find((spirit) => spirit.id === spiritId)?.name ?? spiritId;
      const nowPreferred =
        nextSettings.preferred_persona_ids.includes(spiritId);
      setSystemStatus(
        createApiStatus(
          "persona-db",
          "ready",
          nowPreferred
            ? labels.preferredSpiritSet(spiritName)
            : labels.preferredSpiritCleared(spiritName),
        ),
      );
    } catch (err) {
      setSystemStatus(
        createApiStatus("persona-db", "error", formatUnknownError(err, labels)),
      );
    }
  }
  async function sendMessage(event: React.FormEvent) {
    ...
    if ((proactiveUnreadCounts[activeSpiritId] ?? 0) > 0)
      await acknowledgeProactiveMessages(activeSpiritId);
    ...
    if (aiMessage) {
      try {
        await Promise.all([
          ...
          personaClient.getBondRanking().then(setBondRanking),
          personaClient.getFamiliarityList().then(setFamiliarityList),
  ...
  async function switchToRoom(room: ChatRoom) {
    ...
    releaseFocusedChatRequest();
    try {
      const history = await chatClient.listMessagesForPersona(
        room.id,
        activeSpiritId,
      );
      setActiveRoom(room);
  ...
  async function deleteChatMessage(messageId: string) {
    try {
      ...
      await Promise.all([
        activeSpiritId
          ? refreshMemoryInsight(activeSpiritId)
          : Promise.resolve(),
        refreshMemoryOverview(),
        refreshContextGraph(),
        personaClient.getBondRanking().then(setBondRanking),
  ...
  async function applyPersonaCheatChange(personaId: string | null) {
    ...
    if (
      activeSpiritId &&
      (personaId === null || personaId === activeSpiritId)
    ) {
  ...
  async function updatePersonaCheatPreset(
    personaId: string,
    patch: PersonaCheatPresetPatch,
  ) {
    try {
      setAppSettings(
        await settingsClient.updatePersonaCheatPreset(personaId, patch),
      );
  ...
  async function setMemoryContextEnabled(
    kind: MemoryContextKind,
    enabled: boolean,
  ) {
    try {
      setAppSettings(
        await settingsClient.setMemoryContextEnabled(kind, enabled),
      );
    } catch (err) {
      setSystemStatus(
        createApiStatus("persona-db", "error", formatUnknownError(err, labels)),
  ...
  async function prepareOnDeviceSystemModel(entry: OnDeviceSystemModelEntry) {
    ...
    setModelPreparation({
      model_id: entry.id,
      progress: { ratio: 0, done: false },
      error: null,
    });
    try {
      const catalog = await llmClient.prepareOnDeviceSystemModel(
        entry,
        (progress) => {
      ...
      setModelPreparation({
        model_id: entry.id,
        progress: null,
        error: formatUnknownError(err, labels),
      });
      await refreshModelCatalog();
    }
  }
  ...
  async function installLocalModel(engine: LocalModelEngineKind) {
    ...
    setModelCatalogError(null);
    try {
      const installedModelId = await llmClient.installLocalModel(
        engine,
        (progress) => {
          setModelPreparation({
            model_id: LOCAL_MODEL_INSTALL_PREPARATION_IDS[engine],
            progress,
            error: null,
          });
        },
  ...
  async function downloadLocalModel(entry: LocalModelFileEntry) {
    ...
    setModelCatalogError(null);
    try {
      const installedModelId = await llmClient.downloadLocalModel(
        entry.engine,
        entry.source,
        (progress) => {
          setModelPreparation({
            model_id: LOCAL_MODEL_INSTALL_PREPARATION_IDS[entry.engine],
            progress,
            error: null,
          });
        },
  ...
  async function saveGenerationLimits(
    ...
    try {
      setModelCatalog(
        await llmClient.saveGenerationLimits(
          contextWindowTokens,
          maxOutputTokens,
        ),
      );
  ...
  async function removeLocalModel(entry: LocalModelFileEntry) {
    ...
    try {
      setModelCatalog(
        await llmClient.removeLocalModel(entry.engine, entry.file_name),
      );
      setAppSettings(await settingsClient.get());
      setModelCatalogError(null);
      if (entry.selected) {
  ...
  async function linkBackupDirectory() {
    await runBackupAction(async () => {
      ...
      if (status !== null) {
        ...
        setBackupMessage(
          labels.backupFolderLinked(status.directory_name ?? ""),
        );
      }
    });
  }
  ...
  async function grantBackupDirectoryPermission() {
    await runBackupAction(async () => {
      setBackupDirectoryStatus(
        await syncClient.grantBackupDirectoryPermission(),
      );
    });
  }
  ...
  async function runModuleAction(
    action: () => Promise<ImportedModule[] | null>,
  ) {
    setModuleBusy(true);
    setModuleError(null);
    setModuleMessage(null);
  ...
  async function navigateWorkspace(view: WorkspaceView) {
    ...
    if (view === "memory" || view === "storage") {
      ...
      await Promise.all([
        ...
        view === "memory" && activeSpiritId
          ? refreshMemoryInsight(activeSpiritId)
          : Promise.resolve(),
        view === "memory" && graphPersonaId
          ? viewContextGraphPersona(graphPersonaId)
          : Promise.resolve(),
      ]);
    }
  }
  ...
  const initializeApp = useEffectEvent(async () => {
    ...
    try {
      ...
      initialLanguage = currentSettings.language_configured
        ? currentSettings.language
        : detectBrowserAppLanguage();
      setAppSettings(currentSettings);
      setAppLanguage(initialLanguage);
      needsGate =
        currentSettings.setup_stage !== "done" ||
        !currentSettings.platform_guide_acknowledged;
  ...
  const refreshForRenderedLanguage = useEffectEvent(
    (renderedLanguage: AppLanguage) => {
      const previousLanguage = renderedLanguageRef.current;
      renderedLanguageRef.current = renderedLanguage;
      if (
        previousLanguage === null ||
        previousLanguage === renderedLanguage ||
        appInitializing ||
        platformSupport !== "supported" ||
        appSettings?.setup_stage !== "done"
      ) {
        return;
      }
      void (async () => {
  ...
  const runProactiveConversationCheck = useEffectEvent(async () => {
    if (
      proactiveCheckRunningRef.current ||
      appInitializing ||
      appSettings?.setup_stage !== "done" ||
      appSettings.proactive_messages_enabled === false ||
      !llmStatus?.is_loaded ||
      isTyping
    ) {
      return;
    ...
    try {
      ...
      if (generated) {
        await refreshProactiveUnreadCounts();
        if (
          generated.persona_id === activeSpiritId &&
          generated.room_id === activeRoom?.id
        ) {
          setMessages((current) =>
            current.some((message) => message.id === generated.id)
              ? current
              : [...current, generated],
          );
        }
  ...
  useEffect(() => {
    ...
    const initialTimer = window.setTimeout(
      () => void runProactiveConversationCheck(),
      PROACTIVE_INITIAL_DELAY_MS,
    );
    ...
    appInitializing,
    appSettings?.setup_stage,
    appSettings?.proactive_messages_enabled,
    llmStatus?.is_loaded,
  ]);
  useEffect(() => {
  ...
  const spiritBondSnapshots = spirits.map((spirit) => {
    ...
    const level = entry
      ? computeFamiliarityLevel(entry.familiarity_score).level
      : 1;
  ...
  const stickerEntries: SaviorStickerEntry[] = spiritBondSnapshots.flatMap(
    ({ spirit, detail, assetFolder, level }) =>
  ...
  const saviorName =
    appSettings?.savior_name && appSettings.savior_name.trim().length > 0
      ? appSettings.savior_name
      : labels.saviorDefaultName;
  const saviorProfile: SaviorProfileSnapshot = {
    ...
    highestLevel: spiritBondSnapshots.reduce(
      (highest, snapshot) => Math.max(highest, snapshot.level),
      1,
    ),
    earnedSigils,
    stickerEntries,
    activeModelName:
      appSettings?.active_model && appSettings.active_model.trim().length > 0
        ? appSettings.active_model
  ...
  return {
    workspaceView,
    memoryContextFilter:
      appSettings?.memory_context_filter ?? DEFAULT_MEMORY_CONTEXT_FILTER,
    ...
    moduleMessage,
    activeSkinId:
      appSettings?.persona_skin_ids[activeSpiritId] ?? DEFAULT_SPIRIT_SKIN_ID,
    ...
    relationshipNetwork,
    relationshipNetworkLoading,
    relationshipNetworkActive,
    viewRelationshipNetwork,
    viewRelationshipNetworkPersona,
  };
}
