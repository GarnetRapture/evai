...
const personaArchiveModules = import.meta.glob<SpiritDetail>('/data/personas/*.json', { import: 'default' });
...
const personaArchiveEntries: PersonaArchiveEntry[] = Object.entries(personaArchiveModules)
    .map(([modulePath, load]) => ({ archive_key: archiveKeyFromModulePath(modulePath), load }))
...
export async function loadPersonaPack(archiveKey: string): Promise<SpiritDetail> {
    ...
    const entry = personaArchiveEntries.find((candidate) => candidate.archive_key === archiveKey)
        ?? personaArchiveEntries.find((candidate) => normalizePersonaKey(candidate.archive_key) === normalizedKey);
    if (!entry) {
