...
// 버그 수정 (C-027): 의문형 솔로 라인("왜 대답을 안하지?")은 소형 모델이 맥락 없이
// 그대로 복사해 "맥락을 못 알아듣는" 답변의 주원인이 된다. 후보가 충분하면
// 평서형 라인만 샘플링한다.
const SOLO_QUESTION_LINE_PATTERN = /[?？]\s*[!~.…♡♥♪ㅜㅠㅋㅎ]*$/u;
...
function measureSoloLines(lines: string[]): string[] {
    ...
    const declarative = candidates.filter((line) => !SOLO_QUESTION_LINE_PATTERN.test(line.trim()));
    const pool = declarative.length >= SOLO_LINE_LIMIT ? declarative : candidates;
    if (pool.length <= SOLO_LINE_LIMIT) {
        return pool;
    }
    return Array.from({ length: SOLO_LINE_LIMIT }, (_, index) => {
        const position = Math.round(index * (pool.length - 1) / (SOLO_LINE_LIMIT - 1));
