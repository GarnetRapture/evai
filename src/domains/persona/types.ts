...
export type PersonaPersonalityPresetId =
  | "dataset"
  | "gentle"
  | "cheerful"
  | "tsundere"
  ...
  | "devoted"
  | "bold";
export type PersonaEmotionPresetId =
  | "dataset"
  | "cheerful"
  | "calm"
  | "lovestruck"
  ...
  | "bored"
  | "jealous";
export type PersonaSpeechPresetId =
  | "dataset"
  | "polite"
  | "casual"
  | "affectionate"
...
export type PersonaCheatPresetPatch = Partial<
  Omit<PersonaCheatPreset, "updated_at" | "emotion_applied_at">
>;
...
export type PersonaTemperamentTrait =
  | "warmth"
  | "expressiveness"
  | "initiative";
...
export type PersonaProfileMentionKind =
  | "like"
  | "dislike"
  | "hobby"
  | "speciality";
...
export type PersonaCanonUtteranceKind =
  | "dialogue"
  | "comment"
  | "monologue"
  | "pattern";
...
export type PersonaTeachingTopicId =
  | "identity"
  | "addition"
  | "plants"
  | "variable"
  ...
  | "love"
  | "touch";
export type PersonaTeachingSourceClass =
  | "project_contract"
  | "general_knowledge";
export interface PersonaDatasetIdentity {
  persona_id: string;
  archive_key: string;
  snos: string[];
}
export type PersonaIntimacyLevel = "formal" | "polite" | "casual";
export interface PersonaUserTurnPattern {
  is_question: boolean;
  is_command: boolean;
  is_affection: boolean;
  is_tease: boolean;
  is_shared_feeling: boolean;
  is_self_question: boolean;
  is_savior_state_question: boolean;
  is_identity_challenge: boolean;
  is_boundary_withdrawal: boolean;
  is_out_of_scope_question: boolean;
  is_adult_intent: boolean;
  intimacy_level: PersonaIntimacyLevel | null;
  matched_terms: string[];
}
export interface PersonaTeachingGuidance {
  topic_id: string;
  source_class: string;
  answer: string;
  style_quote: string;
}
export interface PersonaIntimacyGuidance {
  level: string;
  topic: string;
  situation: string;
  spirit_speech: string[];
  spirit_actions: string[];
  savior_speech: string[];
  matched: boolean;
}
export interface PersonaIntimacyDialogue {
  situation: string;
  level: string;
  topic: string;
  spirit_speech: string[];
...
export interface PersonaTeachingExchange {
  role: "user" | "assistant";
  content: string;
}
export interface PersonaTeachingTopic {
  source_class: PersonaTeachingSourceClass;
  fact: string;
  questions: Partial<LocalizedText>;
  paraphrases?: Partial<LocalizedList>;
  history?: Partial<Record<AppLanguage | "zh_tw", PersonaTeachingExchange[]>>;
}
export interface PersonaTeachingStyleSource {
  table: string;
  keys: number[];
  quote: string;
}
export interface PersonaTeachingConversation {
  id: string;
  turns: Partial<Record<AppLanguage | "zh_tw", PersonaTeachingExchange[]>>;
}
export interface PersonaTeachingLesson {
  style_source: PersonaTeachingStyleSource;
  answers: Partial<Record<PersonaTeachingTopicId, Partial<LocalizedText>>>;
  conversations?: PersonaTeachingConversation[];
}
export interface PersonaTeachingLessonFile {
  description: string;
  topics: Record<string, PersonaTeachingTopic>;
  spirits: Record<string, PersonaTeachingLesson>;
}
export interface PersonaJudgmentSource {
  kind: string;
  table: string;
  keys: number[];
  source_class?: string;
...
export interface PersonaJudgmentInterpretation {
...
export type PersonaJudgmentAnnotationKind =
  | "source_grounded_interpretation"
  | "non_self_speech";
export interface PersonaJudgmentRecord {
  record_key: string;
  source: PersonaJudgmentSource;
  annotation_kind: PersonaJudgmentAnnotationKind;
  reviewed_by: string;
  judgment?: PersonaJudgmentInterpretation;
  reason?: string;
  evidence_record_key?: string;
  evidence?: string[];
}
export interface PersonaStoryMemoryRecord {
  text: string;
  story_no: number;
  love_level_min: number;
}
export type PersonaIntimacySegmentKind = "speech" | "action";
export interface PersonaIntimacySegment {
  kind: PersonaIntimacySegmentKind;
  text: string;
}
export interface PersonaIntimacyTurn {
  role: "user" | "assistant";
  segments: PersonaIntimacySegment[];
}
export interface PersonaIntimacyPatternRecord {
  // 정령 SNO (data/dataset/*.json의 id, 예: meryl="2060"). 이 레코드가 배정된 정령.
  spirit_id: string;
  situation: string;
  topic: string;
  level: string;
  turns: PersonaIntimacyTurn[];
}
