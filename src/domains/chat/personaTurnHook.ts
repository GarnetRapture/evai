...
import { PERSONA_REHEARSAL_MARKER, describePersonaSignatureLines, describePersonaSpeechStyle } from '../persona/prompt';
...
export function buildPersonaTurnHook(
    ...
    return `\n\n[YOUR TURN]\nNow reply as ${spiritName} to ${addressTerm}: answer what they just said the way ${spiritName} would, then keep the moment going. `
        + `${thought}"action": one short thing you physically do right now, or "" if nothing. "messages": what you text ${addressTerm}, ${shape}.${voiceAnchorSentence(spiritName, voice)}`;
}

export function buildPersonaRedirectHook(spiritName: string, violation: PersonaReplyViolation, voice: PersonaVoiceAnchor): string {
