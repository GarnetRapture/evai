...
function targetSentence(spiritName: string, addressTerm: string, continuity: PersonaTurnContinuity): string {
    return continuity.latest_user_text === null
        ...
        : `${addressTerm} is with you and has just spoken to you, right above. You react to exactly what they said and did, as "I": what it makes you feel, what you think of it and what you do about it, with ${spiritName}'s own attitude toward ${addressTerm} and ${spiritName}'s own words, sentence endings and rhythm. Whatever ${addressTerm} wants from you, you give it fully first: something they ask you to do, you do now in your action and your words; your reaction or feelings they ask for, you open up and show in several messages; something they want to know about you, you tell from your own life. A deed that names no doer is about what you did, building on what ${addressTerm} already said you did.`;
}
...
export function buildPersonaTurnHook(
    ...
    return `[YOUR TURN]\nYou are ${spiritName} herself, right here with ${addressTerm}. ${targetSentence(spiritName, addressTerm, continuity)} `
        + `This is the next moment of the same scene: what is in your heart, your mood and what you felt when you last spoke carry into it, ${spiritName}'s own personality decides how they show, and [YOUR HEART] sets how much of what you truly feel shows outwardly, how warmly you take in what ${addressTerm} gives you and how much you reach toward them yourself. `
    ...
    return messages.flatMap((message): OnDeviceTextMessage[] => {
        if (message.role === 'assistant') {
            ...
            return envelope.messages.length === 0
                ? []
                : [{ role: 'assistant', content: encodePersonaReplyEnvelope({ ...envelope, action: '' }) }];
        }
