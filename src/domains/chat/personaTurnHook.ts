...
import type { PersonaDialogueExchange, PersonaSpeechRegister, PersonaSpeechStyle } from '../persona/types';
...
export function buildPersonaTurnHook(
    ...
    speechStyle: PersonaSpeechStyle | null,
    voiceRegister: PersonaSpeechRegister | null,
): string {
    ...
    return `\n\n[YOUR TURN]\nNow reply as ${spiritName} to ${addressTerm}: answer what they just said the way ${spiritName} would, then keep the moment going. `
        + `${thought}"action": one short thing you physically do right now, or "" if nothing. "messages": what you text ${addressTerm}, ${shape}.${register}`;
}

export function buildPersonaRedirectHook(spiritName: string, violation: PersonaReplyViolation, voiceRegister: PersonaSpeechRegister | null): string {
    ...
    if (violation === 'register_drift') {
        const registerDescription = describePersonaVoiceRegister(voiceRegister) ?? 'your usual way of talking';
        return `\n\n[REDIRECT]\nYour last draft lost ${spiritName}'s usual way of talking, so it was thrown away. Reply again in ${registerDescription}, with your own vocabulary and rhythm.`;
    }
