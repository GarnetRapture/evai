...
// [출력 정규화 · 버그 수정 금지] 언어 이탈은 language_drift 감지(재생성)로만 다룬다.
// 받아들여진 응답에서 외국어 문자를 삭제하면 "너무 cute해" -> "너무 해"처럼
// 구멍난 문장이 저장·재주입되어 맥락을 오염시킨다. (AI_TRACKING.md C-025)
export function normalizeChatOutput(text: string, language: AppLanguage): string {
    return normalizeLanguageText(removeEmoji(stripMarkupTags(repairHangulComposition(text))), language);
}
