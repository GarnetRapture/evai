...
import {
  type PersonaSpeechRegister,
  type PersonaSpeechStyle,
  type PersonaVoiceAnchor,
} from "../persona/types";
...
import { proofreadPersonaText } from "./personaProofread";
import type {
  PersonaReplyEnvelope,
  PersonaReplyEnvelopeParse,
  PersonaReplyShape,
...
const MESSAGE_QUOTE_WRAPPER_PATTERN =
  /^(?:"([^"]+)"|“([^”]+)”|「([^」]+)」|『([^』]+)』)$/u;
const EMBEDDED_QUOTE_BOUNDARY_PATTERN = /["”」』]\s*[,，、]?\s*["“「『]/u;
const UTTERANCE_EDGE_QUOTE_PATTERN = /^["“「『]+|["”」』]+$/gu;
const ENVELOPE_OBJECT_START_PATTERN =
  /\{\s*"(?:inner_thought|action|messages)"\s*:/u;
const MESSAGE_JSON_TAIL_RESIDUE_PATTERN = /\s*["”]\s*(?:\]\s*\}?|\})\s*$/u;
const MESSAGE_JSON_HEAD_RESIDUE_PATTERN =
  /^\s*\{?\s*"?(?:inner_thought|action|messages)"?\s*:\s*\[?\s*"?/u;
const PERSONA_BREACH_PATTERN =
  /\b(?:AI|A\.I\.|LLM|chat ?bot|language model|assistant|system prompt)\b|인공지능|언어\s*모델|어시스턴트|챗봇|프롬프트|人工智能|语言模型|聊天机器人|提示词/iu;
...
function skipSeparators(source: string, index: number): number {
  ...
  while (
    ...
    JSON_WHITESPACE_PATTERN.test(source[cursor])
  ) {
    cursor += 1;
  }
  return cursor;
...
function coerceEnvelope(value: unknown): PersonaReplyEnvelope | null {
  ...
  const messages = Array.isArray(record[ENVELOPE_KEY_MESSAGES])
    ? (record[ENVELOPE_KEY_MESSAGES] as unknown[]).filter(
        (item): item is string => typeof item === "string",
      )
    : [];
  return {
    inner_thought:
      typeof record[ENVELOPE_KEY_INNER_THOUGHT] === "string"
        ? record[ENVELOPE_KEY_INNER_THOUGHT]
        : "",
    action:
      typeof record[ENVELOPE_KEY_ACTION] === "string"
        ? record[ENVELOPE_KEY_ACTION]
        : "",
    messages,
  };
}
...
function envelopeFromPlainText(text: string): PersonaReplyEnvelope {
  const { actions, spoken } = splitPersonaReplyActions(
    stripReasoning(repairHangulComposition(text)),
    false,
  );
  return {
    inner_thought: "",
...
export function parsePersonaReplyEnvelope(
  text: string,
): PersonaReplyEnvelopeParse {
  ...
  try {
    const envelope =
      objectEnd > objectStart
        ? coerceEnvelope(JSON.parse(text.slice(objectStart, objectEnd + 1)))
        : null;
    if (envelope !== null) {
...
export function removeJsonResidue(message: string): string {
  return message
    .replace(MESSAGE_JSON_TAIL_RESIDUE_PATTERN, "")
    .replace(MESSAGE_JSON_HEAD_RESIDUE_PATTERN, "")
    .trim();
}
...
function unwrapWholeMessageQuotes(message: string): string {
  ...
  return matched === null
    ? message
    : (matched[1] ?? matched[2] ?? matched[3] ?? matched[4]).trim();
}
...
function splitQuotedUtterances(message: string): string[] {
  ...
  return message
    ...
    .map((utterance) =>
      utterance.replace(UTTERANCE_EDGE_QUOTE_PATTERN, "").trim(),
    )
    .filter((utterance) => utterance.length > 0);
}

export function normalizePersonaReplyEnvelope(
  envelope: PersonaReplyEnvelope,
  language: AppLanguage,
  voice: PersonaVoiceAnchor | null = null,
  addressTerm = "",
): PersonaReplyEnvelope {
  const lifted = liftNestedEnvelope(envelope);
  const proofread = (text: string): string =>
    voice === null
      ? text
      : proofreadPersonaText(text, {
          language,
          voice,
          address_term: addressTerm,
        });
  return {
    inner_thought: proofread(
      removeJsonResidue(
        unwrapEmphasisSpans(normalizeChatOutput(lifted.inner_thought, language)),
      ),
    ),
    action: proofread(
      removeJsonResidue(normalizeChatOutput(lifted.action, language)).replace(
        ACTION_WRAPPER_PATTERN,
        "",
      ),
    ).trim(),
    messages: lifted.messages
      .flatMap((message) =>
        splitQuotedUtterances(
          removeJsonResidue(
            unwrapEmphasisSpans(normalizeChatOutput(message, language)),
          ),
        ),
      )
      .map(proofread)
...
export function renderPersonaReplyContent(
  envelope: PersonaReplyEnvelope,
): string {
  const thought =
    envelope.inner_thought.length > 0
      ? `<think>${envelope.inner_thought}</think>`
      : "";
  const lines = [
    envelope.action.length > 0 ? `(${envelope.action})` : "",
    ...envelope.messages,
  ].filter((line) => line.length > 0);
  return `${thought}${lines.join("\n")}`;
}

export function renderPersonaStoredReplyContent(
  envelope: PersonaReplyEnvelope,
): string {
...
export function encodePersonaReplyEnvelope(
  envelope: PersonaReplyEnvelope,
): string {
  return JSON.stringify({
    [ENVELOPE_KEY_ACTION]: envelope.action,
    [ENVELOPE_KEY_MESSAGES]: envelope.messages,
  });
}

export function detectPersonaBreach(envelope: PersonaReplyEnvelope): boolean {
  return PERSONA_BREACH_PATTERN.test(
    [envelope.inner_thought, envelope.action, ...envelope.messages].join("\n"),
  );
}

export function detectPersonaLanguageDrift(
  envelope: PersonaReplyEnvelope,
  language: AppLanguage,
): boolean {
  return containsForeignLanguage(
    [envelope.inner_thought, envelope.action, ...envelope.messages].join("\n"),
    language,
  );
}
...
export function detectPersonaStreamingViolation(
  rawEnvelope: PersonaReplyEnvelope,
  language: AppLanguage,
  spiritName: string,
  ...
  if (isRoleDriftReply(rawEnvelope, spiritName)) {
    return "role_drift";
  }
  return detectPersonaLanguageDrift(rawEnvelope, language)
    ? "language_drift"
    : null;
}
...
function isRepeatedReply(
  ...
  return (
    (cosineSimilarity(
      createLexicalMemoryVector(current),
      createLexicalMemoryVector(previous),
    ) ?? 0) >= REPEATED_REPLY_SIMILARITY
  );
}

function isQuestionOnlyReply(envelope: PersonaReplyEnvelope): boolean {
  return (
    envelope.messages.length >= QUESTION_ONLY_MIN_MESSAGES &&
    envelope.messages.every((message) =>
      QUESTION_ENDING_PATTERN.test(message.trim()),
    )
  );
}
...
function normalizeEchoText(text: string): string {
  return text
    .normalize("NFKC")
    .replace(ECHO_IGNORED_CHARACTER_PATTERN, "")
    .toLocaleLowerCase();
}

function isEchoedMessage(
  message: string,
  normalizedUserText: string,
  userVector: ReturnType<typeof createLexicalMemoryVector>,
): boolean {
  ...
  return (
    normalizedUserText.includes(normalizedMessage) ||
    (cosineSimilarity(createLexicalMemoryVector(message), userVector) ?? 0) >=
      ECHO_SIMILARITY
  );
}

function isEchoUserReply(
  envelope: PersonaReplyEnvelope,
  ...
  const echoedCount = envelope.messages.filter((message) =>
    isEchoedMessage(message, normalizedUserText, userVector),
  ).length;
  return (
    echoedCount > 0 &&
    echoedCount / envelope.messages.length >= ECHO_MESSAGE_RATIO
  );
}

function isDeflectedQuestionReply(
  envelope: PersonaReplyEnvelope,
  latestUserText: string | null,
): boolean {
  return (
...
const SPIRIT_AS_SPEAKER_PATTERN_BUILDERS: ReadonlyArray<
  (name: string) => RegExp
> = [
  (name) =>
    new RegExp(
      `^\\s*(?:\\[|\\(|「|『)?${escapePattern(name)}(?:\\]|\\)|」|』)?\\s*[:：]`,
      "u",
    ),
  (name) =>
    new RegExp(
      `^\\s*${escapePattern(name)}(?:이|가)\\s*(?:말했|대답했|말한다|대답한다|중얼|속삭|말하며|대답하며)`,
      "u",
    ),
];
const ROLE_DRIFT_PATTERNS: readonly RegExp[] = [
  /(?:나는|난|내가|저는|전|제가)\s*(?:남자|남성|아저씨|아빠|아들|소년|남학생)(?:야|이야|입니다|이에요|예요|이다|이지|인데|라서|이므로|라구)(?![가-힣])/u,
  /(?:나는|난|저는|전)\s*(?:구원자|구원자님)(?:이야|야|입니다|이에요|예요|이다|이지|임다|라네)(?![가-힣])/u,
  /(?:i\s*am|i'm|im)\s*(?:a\s*)?(?:man|male|boy|guy|the\s*savior)\b/iu,
  /我(?:是|就是)\s*(?:男人|男性|男生|救援者)/u,
];
// 3인칭 소설 내레이션 감지 (2026-09-22, C-025): 정령은 "I"로 말해야 하며,
// "그녀는 …했다" / "{이름}은 …했다" 같은 소설체는 role_drift로 되돌린다.
const THIRD_PERSON_START_PATTERNS: readonly RegExp[] = [
  /^\s*그녀(?:는|가|을|를|의|와|랑|\s)/u,
  /^\s*그(?:는|가|의|\s)/u,
  /^\s*(?:She|He)\s/u,
];
const NARRATION_ENDING_PATTERN = /(?:다|다요)\s*[.!?…~♡♥♪]*$/u;
const SECOND_PERSON_ADDRESS_PATTERN = /너(?:는|가|를|의)?|네\s|니가|당신|\byou\b/iu;

function escapePattern(value: string): string {
  return value.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
}

function spiritNameSubjectPattern(name: string): RegExp | null {
  return name.trim().length === 0
    ? null
    : new RegExp(`^\\s*${escapePattern(name.trim())}\\s*(?:은|는|이|가)`, "u");
}

function isThirdPersonNarrationLine(
  line: string,
  nameSubjectPattern: RegExp | null,
): boolean {
  const text = line.trim();
  if (text.length < 4 || !NARRATION_ENDING_PATTERN.test(text)) {
    return false;
  }
  if (SECOND_PERSON_ADDRESS_PATTERN.test(text)) {
    return false;
  }
  return (
    THIRD_PERSON_START_PATTERNS.some((pattern) => pattern.test(text)) ||
    (nameSubjectPattern !== null && nameSubjectPattern.test(text))
  );
}

function isRoleDriftReply(
  envelope: PersonaReplyEnvelope,
  spiritName: string,
): boolean {
  const spoken = envelope.messages;
  const speakerPatterns = SPIRIT_AS_SPEAKER_PATTERN_BUILDERS.map((build) =>
    build(spiritName),
  );
  if (
    spoken.some((message) =>
      speakerPatterns.some((pattern) => pattern.test(message)),
    )
  ) {
    return true;
  }
  const nameSubjectPattern = spiritNameSubjectPattern(spiritName);
  // action은 3인칭 무대지시가 정상이므로 내레이션 검사에서 제외한다.
  const narrationLines = [envelope.inner_thought, ...spoken].flatMap((text) =>
    text.split("\n"),
  );
  if (
    narrationLines.some((line) =>
      isThirdPersonNarrationLine(line, nameSubjectPattern),
    )
  ) {
    return true;
  }
  const joined = [envelope.inner_thought, ...spoken, envelope.action].join("\n");
  return ROLE_DRIFT_PATTERNS.some((pattern) => pattern.test(joined));
}
...
export function detectPersonaReplyViolation(
  ...
  if (isRoleDriftReply(envelope, spiritName)) {
    return "role_drift";
  }
  ...
  return detectVoiceRegisterDrift(
    [envelope.inner_thought, ...envelope.messages],
    register,
    language,
  )
    ? "register_drift"
...
export function resolvePersonaReplyMessageLimit(
  ...
  return Math.min(
    PERSONA_REPLY_MAX_MESSAGES,
    Math.max(
      PERSONA_REPLY_MIN_MESSAGES,
      style.messages_per_turn + PERSONA_REPLY_MESSAGE_HEADROOM,
    ),
  );
}

// [핵심 아키텍처 · 수정 금지] 응답 JSON 스키마. 사용자의 명시 지시 없이 변경하지 않는다. (AI_TRACKING.md 5A L-1)
export function buildPersonaReplySpec(
  shape: PersonaReplyShape,
): StructuredReplySpec {
  const properties: Record<string, unknown> = {
    ...(shape.reasoning
      ? { [ENVELOPE_KEY_INNER_THOUGHT]: { type: "string" } }
      : {}),
