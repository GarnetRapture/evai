...
import type {
  ...
  PersonaConversationStateRequest,
} from "./types";
...
function minutesBetween(earlier: string, later: string): number | null {
  ...
  return Number.isFinite(elapsed) && elapsed >= 0
    ? Math.floor(elapsed / MILLISECONDS_PER_MINUTE)
    : null;
}

function findLastSpiritReply(
  history: readonly ChatMessage[],
): ChatMessage | null {
  return (
    history.findLast((message) => {
...
export function analyzeConversationState(
  request: PersonaConversationStateRequest,
): PersonaConversationState {
  const sessionHistory = request.history.filter(
    (message) => message.role === "user" || message.role === "assistant",
  );
  const lastSpiritReply = findLastSpiritReply(sessionHistory);
  const lastMessage = sessionHistory.at(-1);
  const lastSpiritEnvelope =
    lastSpiritReply === null
      ? null
      : envelopeFromStoredReply(lastSpiritReply.content);
  return {
    has_previous_exchange: lastSpiritReply !== null,
    last_spirit_inner_thought:
      lastSpiritReply === null ? "" : extractReasoning(lastSpiritReply.content),
    last_spirit_lines: lastSpiritEnvelope?.messages ?? [],
    last_spirit_action:
      lastSpiritReply === null
        ? ""
        : (lastSpiritReply.spirit_action ?? lastSpiritEnvelope?.action ?? ""),
    last_spirit_asked_question:
      lastSpiritEnvelope !== null &&
      lastSpiritEnvelope.messages.some((line) =>
        QUESTION_MARK_PATTERN.test(line),
      ),
    minutes_since_last_message:
      lastMessage === undefined
        ? null
        : minutesBetween(lastMessage.created_at, request.latest_at),
